...
`extend_current_contract_instance_and_code_ttl` and `extend_contract_instance_and_code_ttl` host functions are meant to extend the TTL of all the necessary entries to keep the current contract accessible and executable. Thus, for the contracts with executable reference, the functions will extend the TTL of the contract instance entry, executable reference entry in the owner contract, and the Wasm code entry referred by the executable reference.
