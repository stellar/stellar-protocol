```
CAP: 0078
Title: Host function for performing limited TTL extensions
Working Group:
...
Add a way to limit the maximum TTL extension for persistent contract data entries.
...
`extend_contract_data_ttl` provides a way to extend a contract data entry TTL by specifying the new value of TTL that entry has to have and a minimum TTL extension threshold for reducing the frequency of TTL extensions in case if the function is called often. This allows developers to specify TTL extension policies such as 'if an entry TTL is less than 29 days, extend it to have 30 days TTL'. While this approach allows distributing the fees among the contract user to some degree, in case if contract usage patterns are uneven, some users may end up paying much more than the others. For example, with the policy example above a user may end up paying for extension anywhere between 1 day and 30 days. More fine-grained control over TTL extension would help the developers that want to prioritize the rent fee stability and fairness.
...
A new version of `extend_contract_data_ttl` function named `extend_contract_data_ttl_v2` is introduced. The new host function is similar to the old version as it allows extending TTL of a contract data entry identified by the key and storage type to the target value, but provides an explicit way to specify the minimum TTL extension (instead of the `threshold` parameter in the old interface), and the maximum TTL extension.
...
index 945ada2c..29053e52 100644
...
- If `TTL_ext_final` exceeds `maxEntryTTL` State Archival network setting value:
     - If storage type `t` is `Persistent`/`Instance`, then `TTL_ext_final` is assigned to be `maxEntryTTL`
...
Minimum extension argument is preserved from the old `extend_contract_data_ttl` function and is just renamed from `threshold` for consistency. Its role is still to reduce the extension frequency, which reduces the user fees (as every TTL update incurs the write fee) and the ledger write load.
...
This CAP does not introduce any backward incompatibilities. `extend_contract_data_ttl` host function will still be supported in all the future protocols as to not break the existing contracts.
