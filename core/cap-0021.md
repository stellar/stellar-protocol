...
duplicate signers, the transaction is rejected with `txNOT_SUPPORTED` (Note that
...
with `TransactionResultCode` `txTOO_LATE`. Otherwise, if all the unsatisfied
...
If none of the above conditions holds (`txTOO_EARLY`,
`txBAD_MIN_SEQ_AGE_OR_GAP`, `txNOT_SUPPORTED`, and `txTOO_LATE` do not apply),
but one of the `extraSigners` is unsatisfied, then the transaction fails with
`txBAD_AUTH`.
