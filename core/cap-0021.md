...
If an account has an `AccountEntryExtensionV1` instead of an
`AccountEntryExtensionV3`, because it hasn't been upgraded yet, then
it behaves as if `seqLedger` and `seqTime` are both 0.
...
struct AccountEntryExtensionV3
{
    ...
    AccountEntryExtensionV2 *sponsorInfo;
...
signed values.  Since `TimePoint`s should only be positive, any
...
Because `GeneralPreconditions` specifies multiple pre-conditions,
there may be multiple reasons why a transaction is invalid.  If any of
the reasons is permanent---for instance the `maxTime` or `maxLedger`
has been exceeded--then the transaction is rejected with
`TransactionResultCode` `txTOO_LATE`.  Otherwise, if all the
invalidating preconditions are based on values that could later be
valid, then the transaction is rejected with `txTOO_EARLY`.  If an
account still has an AccountEntryExtensionV1

### XDR diff

```diff mddiffcheck.base=v16.0.0
diff --git a/src/xdr/Stellar-ledger-entries.x b/src/xdr/Stellar-ledger-entries.x
index 96037b74..aa6b15df 100644
...
@@ -128,6 +129,9 @@ const MAX_SIGNERS = 20;
...
+
 ...
 {
     uint32 numSponsored;
@@ -142,6 +146,19 @@ struct AccountEntryExtensionV2
     ext;
 };
...
+
 ...
 {
     Liabilities liabilities;
@@ -189,6 +206,8 @@ struct AccountEntry
         void;
     case 1:
         AccountEntryExtensionV1 v1;
+    case 3:
...
@@ -332,10 +351,10 @@ case CLAIM_PREDICATE_OR:
...
index 6aa8fa10..8ff58d05 100644
