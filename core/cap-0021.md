...
Note that `extraSigners`, because they are added to the `signatures`
field of a `TransactionV1Envelope`, consume some of the 20 signature
slots.  An account that requires 20 signatures on transactions cannot
therefore make use of transactions with a non-empty `extraSigners`.
...
All transactions are validated twice.  In a first pass, transactions
are validated and fees are charged.  If a transaction fails validation
in this pass, it cannot be charged a fee (but should not have been
included in the block in the first place).  The first phase does not
need to check sequence numbers, because transactions with invalid
sequence numbers should no be included in a block in the first place.
In the second phase, each transaction is validated, its sequence
number increased, and then its operations executed.  A consequence of
these two phases is that a `BUMP_SEQUENCE` operation can make a
transaction fail validation in the second phase after it has already
paid a fee in the first phase.  Whenever a transaction fails
validation in the second phase, the `sourceAccount` simply loses the
fee.
...
reasons is permanent---for instance the `maxTime` or `maxLedger` has
been exceeded--then the transaction is rejected with
`TransactionResultCode` `txTOO_LATE`.  Otherwise, if all the
unsatisfied preconditions are based on values that could later be
valid, then the transaction is rejected with `txTOO_EARLY`.
