...
Note we add a signed `Duration` type, used by `minSeqAge`.
...
valid (the `minTime` or `minLedger` have not been reached yet), then
the transaction is rejected with `txTOO_EARLY`. If the failure is due
to `minSeqAge` or `minSeqLedgerGap`, then the transaction is rejected
with the new `txBAD_MIN_SEQ_AGE_OR_GAP` error code.  If neither of the
above conditions holds (`txTOO_EARLY`, `txBAD_MIN_SEQ_AGE_OR_GAP`, and
`txTOO_LATE` do not apply), but one of the `extraSigners` is unsatisfied,
then the transaction fails with `txBAD_AUTH`.

### XDR diff

```diff mddiffcheck.base=v17.3.0
diff --git a/src/xdr/Stellar-ledger-entries.x b/src/xdr/Stellar-ledger-entries.x
index c870fe09a..5c772f1c1 100644
...
index 1a4e491a1..2faae820d 100644
...
@@ -1508,7 +1560,8 @@ enum TransactionResultCode
...
@@ -1537,6 +1590,7 @@ struct InnerTransactionResult
