...
```
/// Constructs a BigInt from an u64. Traps if the value cannot be represented by BigInt.
func $bigint_from_u64(param $val u64) (result i64)

/// Converts the value of $obj_a to an u64. Traps if the value cannot be represented by an u64.
func $bigint_to_u64(param $obj_a i64) (result u64)

/// Constructs a BigInt from an i64. Traps if the value cannot be represented by BigInt.
...
/// Calculates ($obj_a ^ $obj_e) mod $obj_m. Note that this rounds like mod_floor, not like the % operator, which makes a difference when given a negative self or modulus. 
/// The result will be in the interval [0, modulus) for modulus > 0, or in the interval (modulus, 0] for modulus < 0.
/// Traps if the exponent is negative or the modulus is zero.
...
`SCO_MAP`, and `SCO_BINARY` where size of bin is greater than 32 bytes. The
`SCO_BINARY` 32 byte limit is sufficient for hashes and ed25519 keys.
...
/// SCO_MAP. If the value is SCO_BINARY, size of bin is limited to 32 bytes.  
...
+{
+    SCSymbol label;
+    SCVal value; //Cannot be SCO_VEC or SCO_MAP. If SCO_BINARY, size of bin is limited to 32 bytes
+}
+
