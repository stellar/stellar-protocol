...
```
CAP: 0023
Title: Two-Part Payments with PrepareTrustEntry and ClaimableBalanceEntry
...
1. Fail with `CREATE_CLAIMABLE_BALANCE_LOW_RESERVE` if the `sourceAccount` does
   not have at least `max(1, claimants.size()-1) * baseReserve` available
   balance of native asset
