...
The de facto standard Soroban UDT `contracttype` structs implementation is is based on a `MapObject` with `Symbol` keys, backed by the two host functions `map_new_from_linear_memory` and `map_unpack_to_linear_memory` that allow to convert UDT structs to and from `MapObject`s respectively. However, these host functions do strict validation and cause the contract call to trap if the input/output map doesn't *exactly* match the expected schema (i.e. if it doesn't have the exact same keys as the struct). This makes it impossible to migrate UDTs to a new schema, because the new schema may have additional keys, or may not have some of the old keys.

This is problematic for the most of the possible UDT-based data migration scenarios: 

  - A contract may not add a new field to a UDT struct or remove the old unused field from it when its implementation is updated. There are known cases where this has completely 'bricked' the contracts on update
  - A struct used in the function interface may not be updated unless all the interface user contracts are updated at the same time atomically, which is not feasible if multiple contract instances share the same interface
  - A contract may not even easily try to verify if the map has the expected schema without manually iterating it, as the existing host functions never return gracefully and panic on every error
...
Keys array must consist of `Symbol` values sorted in the ascending order. Values array may contain arbitrary `Val`s. If the keys are not sorted in the ascending order, or if the key and value arrays have different lengths, the function panics.
...
Keys array must consist of `Symbol` values sorted in the ascending order. If the keys are not sorted in the ascending order, or if the key and value arrays have different lengths, the function panics.
...
SDK may adopt the new host functions as soon as they're available, and then the developers will be able to use the migration-friendly behavior in a more explicit way - even if there are some subtle issues with the new logic, they will able to test it first before mainnet deployment (unlike in case of an unconditional change of the existing host functions).
...
Consider a protocol where a multiple contracts consult the same policy contract to perform some checks. The policy contract has the following function for checking the policy:
