...
This CAP aims to solve this issue and allow for the parallelization that has a guaranteed upper bound for the execution time of transaction sets (given at least the minimum specified number of physical threads supported).
...
The effective fee for a given transaction in `ParallelTxsComponent` is computed in the following way:
  ...
  * If `baseFee` is set, then transaction's effective fee is set the value of `baseFee`
