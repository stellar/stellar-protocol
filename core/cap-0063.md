...
#### Move fee refund to after the transactions in a stage are applied.

Currently, the fee refund is applied after every transaction. This CAP specifies a change to instead apply refunds after each stage. The meta will also need to be updated. Currently, the soroban fee refund currently exist in `txChangesAfter` under `TransactionMeta`. The refund will stop appearing in this field, and will instead be emitted through `postTxApplyFeeProcessing`, which is in the new `TransactionResultMetaV1` struct, which will be emitted by the new `LedgerCloseMetaV2` struct.
