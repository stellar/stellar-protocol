...
Note that no event will be emitted if the `amount` is 0.
...
Note that it is possible for no event to be emitted in the case where the merged account has a 0 balance. This is possible if that account has its base reserves sponsored. In that case, there is no movement of value.
