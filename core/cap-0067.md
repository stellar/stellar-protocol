...
An additional `fee` event will be emitted if there is a refund, where the amount is a negative value, indicating that `from`'s balance will be increased by the amount. This second `fee` event will only appear if a refund occurs, so classic transactions and Soroban transactions that do not have a refund will only emit one `fee` event.
...
As specified in the [section to update the transfer and mint events data field](#emit-a-map-as-the-data-field-in-the-transfer-and-mint-event-if-muxed-information-is-being-emitted-for-the-destination), `to_muxed_id` can be added to the `data` map on the `transfer` and `mint` event. More specifically, if the source account is muxed and appears `to` in an event, the respective `to_muxed_id` will be set. For the final event to the destination `to_muxed_id` will be set if the `destination` is muxed, and if not, then it'll be set if the tx memo is set.
...
As specified in the [section to update the transfer and mint events data field](#emit-a-map-as-the-data-field-in-the-transfer-and-mint-event-if-muxed-information-is-being-emitted-for-the-destination), `to_muxed_id` can be added to the `data` map on the `transfer` and `mint` event. More specifically, if the source account is muxed and appears as `to` in an event, `to_muxed_id` will be set.
...
Due to the fact that `from` and `to` are liquidity pool id's and claimable balance id's respectively, we will not emit `to_muxed_id` in the `transfer` event.
...
As specified in the [section to update the transfer and mint events data field](#emit-a-map-as-the-data-field-in-the-transfer-and-mint-event-if-muxed-information-is-being-emitted-for-the-destination), `to_muxed_id` can be added to the `data` map. More specifically, it can be set if the transaction memo is set.
