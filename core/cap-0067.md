...
#### Consuming fee events in chronological order

While the fee events appear within `TransactionMetaV4`, consuming those events in chronological order will require some additional logic - 

* The initial fee is always charged before any transaction has being applied, so the first `fee` event in every `TransactionMetaV4` should be consumed first.
* Before p23, the refund occurs immediately after the transaction applies, before the next transaction applies. The refund (if one occurred) is the second `fee` event, which must be a negative value. This should be consumed right after the related transaction is applied.
* Starting from p23, the refund occurs after all transaction have finished being applied. The refund (if one occurred) is the second `fee` event, which must be a negative value. This should be consumed after all transactions have been applied.
