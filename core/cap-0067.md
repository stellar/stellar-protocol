...
`transfer_with_memo(from: Address, to: Address, amount: i128, from_muxed_id: Option<u64>, to_muxed_id: Option<u64>)` and extend the `transfer` event topics to `["transfer", from: Address, to: Address, sep0011_asset: String, from_muxed_id:Option<u64>, to_muxed_id:Option<u64>]`.

The `transfer_with_memo` function will forward the muxed id's to the two new topics on the `transfer` event. If the optional muxed id is not specified, then a `SCV_VOID` `SCVal` will be emitted instead. The muxed parameters must be passed to the `transfer_with_memo`'s `require_auth` call.
