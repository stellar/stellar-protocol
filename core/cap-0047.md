...
index 23918226d..96fb43a68 100644
...
[CAP-0053](https://github.com/stellar/stellar-protocol/blob/master/core/cap-0053.md) with
...
salt provided by the user. The `create_contract` host function also does some
...
//     sha256("create_contract(nonce: u256, contract: Vec<u8>, salt: u256, key: u256, sig: Vec<u8>)"),
// Calls check with
//     nonce = nonce
...
fn create_contract(nonce: u256, contract: Vec<u8>, salt: Vec<u8>, key: u256, sig: Vec<u8>) -> ContractID;
```
...
`InvokeHostFunctionOp` for simplicity. The `create_contract_using_parent_id`
...
`create_contract_using_parent_id`.

`create_contract_using_parent_id` will trap if it is not called from within
another contract.
...
You can either use `create_contract` or `create_contract_using_parent_id`.
`create_contract` is needed if it is called from `InvokeHostFunctionOp` since
there is no parent contract (and therefore no parent contract ID). It is also
helpful in situations where the salt is provided by the user in a factory
contract. `create_contract` uses a ED25519 with signature verification, so you
can guarantee your contract ID won't be taken. `create_contract_using_parent_id`
can only be used from within another contract, but it is much simpler since it
just uses the parent contractID and a salt.
