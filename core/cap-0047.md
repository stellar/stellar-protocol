...
index 3eb578f16..c5c0fed65 100644
...
+    ENVELOPE_TYPE_POOL_REVOKE_OP_ID = 7,
+    ENVELOPE_TYPE_CONTRACT_ID_FROM_KEY = 8,
...
index 23918226d..7e555ccd5 100644
...
+case ENVELOPE_TYPE_CONTRACT_ID_FROM_KEY:
+    struct
+    {
+        uint256 key;
+        uint256 salt;
...
(https://github.com/stellar/stellar-protocol/blob/master/core/cap-0052.md).

`create_contract` will create a new `ContractDataEntry`
...
`ENVELOPE_TYPE_CONTRACT_ID_FROM_KEY`, which contains a public key, and a salt
provided by the user. The `create_contract` host function also does some
validation to ensure the contract and contractID are created securely. It uses
the `check` functions and `Identifier` defined in CAP-0052 to verify the
identity of the contract creator. 
...
TODO: Another envelope type for contractIDs from accountIDs.

```rust
// Define CREATE_CONTRACT_SEPARATOR =
//     sha256("create_contract(nonce: u256, contract: Vec<u8>, salt: u256, key: u256, creator: Identifier, sig: Vec<u8>)"),
// Calls check with
//     nonce = nonce
//     params = vec![CREATE_SEPARATOR, salt, creator, sha256(contract)]
//     key = key
//     sig = sig
// Creates a new contract.
fn create_contract(nonce: u256, contract: Vec<u8>, salt: Vec<u8>, key: u256, creator: Identifier, sig: Vec<u8>) -> ContractID;

// Same as above but replaces the single key and sig with an AccountID
// and a list of signatures. For account based authorization.
fn create_contract(nonce: u256, contract: Vec<u8>, salt: Vec<u8>, key: AccountID, creator: Identifier, sig: Vec<(u256, Vec<u8>)>)) -> ContractID;
```

### Creating a contract within a contract
Factory contracts are quite popular already on other networks, so this CAP adds
functionality to support them. The contractID for these contracts can be
generated a little differently than a contract created using
`InvokeHostFunctionOp` for simplicity. The `create_contract_using_contractID`
call will create the new contractID by taking the SHA-256 hash of a
`HashIDPreimage`, with type `ENVELOPE_TYPE_CONTRACT_ID_FROM_CONTRACT`, which
contains the parent contractID, and a salt that the parent contract passes to
`create_contract_using_contractID`.

`create_contract_using_contractID` will trap if it is not called from within
another contract.

Contract creators still have the option to use `create_contract` if they choose to.
This can be helpful in factory contracts where the salt is specified by the
contract caller.

```rust
// Can only be used inside another contract. Will use the parents contractID.
fn create_contract_using_parent_id(contract: Vec<u8>, salt: Vec<u8>) -> ContractID
```
...
contractID. Creating a contract using `InvokeHostFunctionOp` uses the source
account and a salt, while contracts created within other contracts use the
parent contractID and a salt.
