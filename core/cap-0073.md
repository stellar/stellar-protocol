...
This CAP introduces a new Stellar Asset contract function `allow_trust` for creating trustlines. The new creates an unlimited trustline for a G-account in case if it's missing. The trustline creation semantics are the same as for the existing `ChangeTrustOp`. Account authorization is required for actually creating the trustline, but not for calling `allow_trust`.
...
///
/// In case if the trustline is actually created, this will require 
