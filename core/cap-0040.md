...
The ed25519 signed payload signer can be used to make operations on Stellar and 
another chain atomic. This is limited to other chains that utilize the ed25519 
signature scheme and whose operations are authorized by signing a maximum of 32-
bytes.

An ed25519 signer can be added as a signer to a Stellar account also with a 
weight lower than any threshold, but high enough such that signatures by both 
will authorize the transaction. An ed25519 signed payload signer can be 
constructed using a ed25519 public key that can authorize a operation on the 
other chain, and using a payload that if signed would authorize an operation on 
that other chain. The signer is added to a Stellar account with a weight lower 
than any threshold but high enough such that together with the ed25519 signer the 
Stellar transaction will be authorized. The operator the Stellar account signs 
the transaction with the ed25519 public key and shares the transaction with the 
...
efficient since the ed25519 signed payload signer can be defined without 
interacting with the network.

This signer is limited to signing operations on other chains that use the ed25519 
signature scheme, and to signing payloads that are a maximum of 32-bytes. The 
addition of other variations of this signer that utilize other signature schemes, 
such as ECDSA signatures on the secp256k1 curve, would make this type of signer 
useful with a wider variety of chains. This proposal is limited to the ed25519 
signature scheme because the primary use case is the signing of Stellar 
transaction hashes.
