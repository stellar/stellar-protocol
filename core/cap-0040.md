...
It is possible for an attacker to convince someone to unknowingly sign a hash
which corresponds to a `TransactionSignaturePayload` for an account they
control. This is only possible when the payload is 32 bytes long. To avoid these
attacks, people should never sign a 32 byte payload unless they know the
preimage.
