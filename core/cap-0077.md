...
The transactions with hashes in `freezeBypassTxs` are allowed to access the frozen ledger keys at *validation time* without being rejected by the protocol. `freezeBypassTxs` setting is ignored at apply time, i.e. it's not possible to bypass the freeze for the transactions that access frozen keys via the operations with opaque source/destination (claimable balance and liquidity pool operations) or via DEX.
