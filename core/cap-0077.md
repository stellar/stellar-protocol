...
In case if a transaction accesses a frozen key, the protocol will check if the transaction hash is in the `unfrozenTxHashes` setting, and if it is, then the transaction will be considered valid.
