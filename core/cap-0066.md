...
Since the write fee is based on the Soroban state now, it does not make sense to charge the same fee for writing the classic entries. Moreover, protocol currently doesn't provide any way to increase the size of the classic state, as only the balance-changing the operations are currently supported. This is why
