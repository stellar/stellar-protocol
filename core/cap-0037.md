...
- Maximum allowed price deviation is controlled by the `priceAccuracy` (***d***)
  parameter.  
  $\left\vert 1-\Large\frac{r}{P}\right\vert\leq\Large\frac{1}{d}$  
...
- Effective token amounts that can be deposited are adjusted to follow the
  ...
  $a=\left\lceil{b_d\cdot P}\right\rceil$
  otherwise  
  $b=\left\lceil\Large\frac{a_d}{P}\right\rceil$\
  where:
    - $a_d=min(maxAmountA,accountBalanceA)$
    - $b_d=min(maxAmountB,accountBalanceB)$
...
- Stake weight calculated as  
  $s=\left\lfloor{S\cdot\sqrt{\Large\frac{a\cdot b}{A\cdot B}}}\right\rfloor$\
  where
...
- If `LiquidityPoolEntry` does not exist on-chain (this is the first deposit),
  ...
  $s=\left\lfloor{\sqrt{a\cdot b}}\right\rfloor$
...
- Effective pool fee recalculated using a weighted average formula:
  $F_n=\Large\frac{S\cdot F + s\cdot f}{S+s}$\
  where
    - ***Fₙ*** - new pool fee
    - ***S*** - total stakes currently in the pool (from `LiquidityPoolEntry`)
    - ***F*** - current pool fee
...
- An account has a right to withdraw liquidity stake equal to  
  ...
  $a=\left\lfloor{A\cdot l}\right\rfloor$
  $b=\left\lfloor{B\cdot l}\right\rfloor$\
  where
    - ***s*** - share to withdraw
...
- Price-bound swap (estimate based on the target price)  
  ...
  $a=\left\lceil{\left(\sqrt{A\cdot B\cdot P}-A\right)\cdot(1+f)}\right\rceil$
  $b=\left\lceil{\Large\frac{A+a}{P}\scriptsize-B}\right\rceil$

  If *new price* < *current price*:  
  $b=\left\lceil{(\Large\sqrt\frac{A\cdot B}{P}\scriptsize-B)\cdot(1+f)}\right\rceil$
  $a=\left\lceil{(B+b)\cdot P-A}\right\rceil$
- Amount-bound buy swap (estimate based on the target token amount)  
  For a given token A target amount:  
  $b=\left\lceil{\Large\frac{a\cdot B\cdot (1+f)}{A-a}}\right\rceil$

  For a given token B target amount:  
  $a=\left\lceil{\Large\frac{b\cdot A\cdot (1+f)}{B-b}}\right\rceil$
- Amount-bound sell swap (estimate based on the source token amount)  
  For a given token A source amount:  
  $b=\left\lfloor{\Large\frac{-a\cdot B\cdot (1-f)}{A+a}}\right\rfloor$

  For a given token B source amount:  
  $a=\left\lfloor{\Large\frac{-b\cdot A\cdot (1-f)}{B+b}}\right\rfloor$\
  where
