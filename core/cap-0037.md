...
- Stake weight calculated as  
  $s=floor(S\cdot\sqrt{\Large\frac{a\cdot b}{A\cdot B}})$  
  where
...
- Price-bound swap (estimate based on the target price)  
  ...
  $a=ceil((\sqrt{A\cdot B\cdot P}-A)\cdot(1+f))$  
  $b=ceil(\Large\frac{A+a}{P}\scriptsize-B)$  
  If *new price* < *current price*:  
  $b=ceil((\Large\sqrt\frac{A\cdot B}{P}\scriptsize-B)\cdot(1+f))$  
  $a=ceil((B+b)\cdot P-A)$
- Amount-bound buy swap (estimate based on the target token amount)  
  For a given token A target amount:  
  $b=ceil\Large(\frac{a\cdot B\cdot (1+f)}{A-a})$  
  For a given token B target amount:  
  $a=ceil\Large(\frac{b\cdot A\cdot (1+f)}{B-b})$
- Amount-bound sell swap (estimate based on the source token amount)  
  For a given token A source amount:  
  $b=floor\Large(\frac{-a\cdot B\cdot (1-f)}{A+a})$  
  For a given token B source amount:  
  $a=floor\Large(\frac{-b\cdot A\cdot (1-f)}{B+b})$  
  where
