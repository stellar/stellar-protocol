...
+{
+    uint32 poolType;// pool invariant identifier
...
@@ -479,6 +520,20 @@ case CLAIMABLE_BALANCE:
     {
...
+    WITHDRAW_POOL_LIQUIDITY = 22
 ...
 /* CreateAccount
@@ -390,6 +392,38 @@ struct ClawbackClaimableBalanceOp
     ClaimableBalanceID balanceID;
 };
...
+{
+    int32 poolType; // pool invariant identifier
+    Asset assetA;   // asset A of the liquidity pool
+    Asset assetB;   // asset B of the liquidity pool
...
- Stake weight calculated as  
  ![`s=a*b*S/(A*B)`](https://render.githubusercontent.com/render/math?math=s%3D%5CLarge%5Cfrac%7Ba%20%5Ccdot%20b%20%5Ccdot%20S%7D%7BA%20%5Ccdot%20B%7D)  
  where
    - ***s*** - share of the pool an account obtains after the deposit
    - ***a*** and ***b*** - effective amount of tokens to deposit
    - ***S*** - total stakes currently in the pool (stored
...
- If `LiquidityPoolEntry` does not exist on-chain (this is the first deposit) it
  is created automatically. The stake weight for the deposit, in this case, is
  computed as  
  ![`s=min(a,b)`](https://render.githubusercontent.com/render/math?math=s=min(a,b))
- If the account `LiquidityStakeEntry` exists, the stake is increased
  `stake`+=***s***. Otherwise, new `LiquidityStakeEntry` created.
...
- `LiquidityStakeEntry` updated: `stake`-=***s***. If the remaining stake equals
  zero, `LiquidityStakeEntry` is removed and `numSubEntries` for the source
...
in permile (‰)  
![`f=x*λ/1000`](https://render.githubusercontent.com/render/math?math=f%3Dx%5Ccdot%5CLarge%5Cfrac%7B%26lambda%3B%7D%7B1000%7D)    
where

- ***f*** - the fee charged
- ***x*** - amount of tokens sent to the pool
- ***λ*** - `liquidtyPoolYield` value
...
- Price-bound swap (estimate based on the target price)  
  ...
  If *new price* < *current price*:  
  ![`b=ceil((√(A*B/P)-B)*(1+f))`](https://render.githubusercontent.com/render/math?math=b%3Dceil((%5Csqrt%5CLarge%5Cfrac%7BA%5Ccdot%20B%7D%7BP%7D%5Cscriptsize-B)%5Ccdot(1%2Bf)))  
  ![`a=ceil((B+b)*P-A)`](https://render.githubusercontent.com/render/math?math=a%3Dceil((B%2Bb)%5Ccdot%20P-A))
- Amount-bound buy swap (estimate based on the target tokens amount)  
...
- Amount-bound sell swap (estimate based on the source tokens amount)  
...
The `poolType` identifier present in new operations and ledger entries provides
...
The proportion of swap fees charged by a pool can be a subject of changes in the
future. The easiest way to provide the flexibility of this parameter without the
need to upgrade the protocol is to add `liquidtyPoolYield` parameter to the
`LedgerHeader` entry. Validators can vote on changing the yield proportion. This
parameter can be also utilized in a combination with normalization coefficients
for other pool types as well.
