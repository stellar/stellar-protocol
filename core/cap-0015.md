...
while preserving binary compatibility. `TransactionV1` is introduced with
corresponding envelope type `ENVELOPE_TYPE_TX_V1` to recover the `AccountID`
union. `FeeBumpTransaction` is introduced with corresponding envelope type
`ENVELOPE_TYPE_TX_FEE_BUMP` as a new type of transaction.
...
enum EnvelopeType
{
    ...
    ENVELOPE_TYPE_TX_V1 = 5,
    ENVELOPE_TYPE_TX_FEE_BUMP = 6
};
...
enum TransactionResultCode
{
    ...
    txINNER_TX_INVALID = -13     // fee bump inner transaction invalid, never
...
#### Validity
Validity requirements for a `TransactionEnvelope` of type `ENVELOPE_TYPE_TX_V0`
or `ENVELOPE_TYPE_TX_V1` are identical to the existing validity requirements.

Checking the validity of a `TransactionEnvelope E` of type `ENVELOPE_TYPE_TX_FEE_BUMP`
with inner transaction envelope `F = E.feeBump().tx.innerTx` returns

- `txTOO_EARLY` if `E.feeBump().tx.timeBounds` is specified and
  `E.feeBump().tx.timebounds.minTime` exceeds the ledger close time
- `txTOO_EARLY` if `E.feeBump().tx.timeBounds` is specified and
  `E.feeBump().tx.timebounds.maxTime > 0` and the ledger close time exceeds
  `E.feeBump().tx.timebounds.maxTime`
...
- `txINNER_TX_INVALID` if `F` is invalid, unless the only reason that `F` is invalid
...
replace `F` with `E` if and only if the fee rate for `E` is at least 2 times the
fee rate for `F`.
...
### ENVELOPE_TYPE_TX_V1
We have chosen to use `ENVELOPE_TYPE_TX_V1` instead of `ENVELOPE_TYPE_TX` for
a `TransactionEnvelope` containing a `Transaction` in order to ensure that
changing the type of a `TransactionEnvelope` changes the required signatures.
The main reason for this is to simplify the possibility of having a "canonical"
or "normalized" XDR representation of a transaction, a concept that has arisen
during the discussion of some CAPs in the past. If we removed
`ENVELOPE_TYPE_TX_V1` then specifying a canonical representation would require
designating `ENVELOPE_TYPE_TX` as the preferred representation. By introducing
`ENVELOPE_TYPE_TX_V1` we can avoid this issue entirely should we ever require a
canonical XDR representation of a transaction.
...
we may as well return the results of the inner transaction in place of the
results of the outer transaction (which would always just indicate that the
outer transaction was applied successfully).

## Backwards Incompatibilities
Downstream systems that submit transactions will continue to function, but
downstream systems that process historical transactions will need to be updated
with the new XDR.
