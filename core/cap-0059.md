...
```
{
    ...
    "return": "BytesObject",
    "docs": "Adds two BLS12-381 G1 points given in bytes format and returns the resulting G1 point in bytes format. G1 serialization format: `be_encode(X) || be_encode(Y)` and the most significant three bits of X encodes flags, i.e.  bits(X) = [compression_flag, infinity_flag, sort_flag, bit_380, .. bit_0]",
...
},
{
    ...
    "return": "BytesObject",
    "docs": "Adds two BLS12-381 G2 points given in bytes format and returns the resulting G2 point in bytes format. G2 serialization format: be_encode(X_c1) || be_encode(X_c0) || be_encode(Y_c1) || be_encode(Y_c0), and the most significant three bits of X_c1 are flags i.e. bits(X_c1) = [compression_flag, infinity_flag, sort_flag, bit_380, .. bit_0]",
    "min_supported_protocol": 22
},
{
    "export": "a",
...
},
{
    ...
    "return": "BytesObject",
    "docs": "Maps a BLS12-381 quadratic extension field element (Fp2) to G2 point. Fp2 serialization format: be_encode(c1) || be_encode(c0)",
    "min_supported_protocol": 22
},
{
    "export": "d",
...
##### Operations on G1
- `bls12_381_g1_add` - perform point addition in G1. Cost: covers the cost of decoding (`Bls12381DecodeFp`) and validating (`Bls12381G1Validate`) G1 points, point addition (`Bls12381G1Add`), conversion of from projective to affine space (`Bls12381G1ProjectiveToAffine`), and encoding the result to bytes `Bls12381EncodeFp`. 
- `bls12_381_g1_mul` - perform scalar multiplication in G1. Cost: includes decoding G1 point, converting `fr` from `U256` (`Bls12381FrFromU256`), point multiplication `Bls12381G1Mul`, converting the point from project to affine and encoding the result into bytes.
- `bls12_381_g1_msm` - perform multi-scalar-multiplication (MSM) in G1. Cost: includes decoding of the G1 vector, the fr vector, and the MSM operation `Bls12381G1Msm`, and encoding of the resulting G1 point. Errors: if the two vectors have different lengths, or the length is zero.
- `bls12_381_map_fp_to_g1` - maps an `fp` to a point in `G1`. Cost includes decoding of the G1 point, the mapping cost `Bls12381MapFpToG1`, and encoding of the resulting G1 point.
...
##### Operations on G2
- `bls12_381_g2_add` - perform point addition in G2. Cost: covers the cost of decoding and validating (`Bls12381G2Validate`) G2 points, point addition (`Bls12381G2Add`), conversion of from projective to affine space (`Bls12381G2ProjectiveToAffine`), and encoding the result to bytes.
- `bls12_381_g2_mul` - perform scalar multiplication in G2. Cost: includes decoding G2 point, converting `fr` from `U256`, point multiplication `Bls12381G2Mul`, converting the point from project to affine and encoding the result into bytes.
- `bls12_381_g2_msm` - perform multi-scalar-multiplication (MSM) in G2. Cost: includes decoding of the G2 vector, the fr vector, and the MSM operation `Bls12381G2Msm`, and encoding of the resulting G2 point. Errors: if the two vectors have different lengths, or the length is zero.
- `bls12_381_map_fp2_to_g2` - maps an `fp2` to a point in `G2`. Cost includes decoding of the G2 point, the mapping cost `Bls12381MapFp2ToG2`, and encoding of the resulting G2 point.
- `bls12_381_hash_to_g2` - hashes a message (a sequence of bytes) into a point in `G2`. Cost is covered by `Bls12381HashToG2`. The domain separation tag (dst) is "Soroban-V00-CS00-with-BLS12381G2_XMD:SHA-256_SSWU_RO_".
...
##### Operations on Fr
- `bls12_381_fr_add` - performs addition `(lhs + rhs) mod r` between two `fr`. Cost: conversion of fr from U256 (`Bls12381FrFromU256`), scalar addition `Bls12381FrAddSub`.
- `bls12_381_fr_sub` - performs subtraction `(lhs - rhs) mod r` between two `fr`. Cost: conversion and scalar subtraction `Bls12381FrAddSub`.
- `bls12_381_fr_mul` - performs multiplication `(lhs * rhs) mod r` between two `fr`. Cost: conversion and scalar multiplication `Bls12381FrMul`.
- `bls12_381_fr_pow` - performs exponentiation `lhs.exp(rhs) mod r` between `fr` and an u64 exponent. Cost: conversion and scalar exponentiation `Bls12381FrPow`.
- `bls12_381_fr_inv` - performs inversion on `fr`. Cost: conversion and scalar inversion `Bls12381FrInv`.
