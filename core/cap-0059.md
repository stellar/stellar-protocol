...
## Motivation
Pairing friendly elliptic curve operations are the backbone of many advanced Zero Knowledge (ZK) constructions, with wide array of applications ranging from scaling, to identity management and privacy preserving solutions. They are crucial for unlocking zero knowledge functionality in smart contracts.
...
- The compression flag (the most significant bit, i.e. bit 0 in the bytes array) is set. 
...
**Description**: hashes a message (a sequence of bytes) into a point in `G1`. The domain separation tag (dst) is "Soroban-V00-CS00-with-BLS12381G1_XMD:SHA-256_SSWU_RO_".
 ...
 **Description**: hashes a message (a sequence of bytes) into a point in `G2`. The domain separation tag (dst) is "Soroban-V00-CS00-with-BLS12381G2_XMD:SHA-256_SSWU_RO_".
 
 **Cost**: covered by `Bls12381HashToG2`. 
