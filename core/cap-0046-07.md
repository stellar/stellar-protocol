...
`min_historical_fee(txEnvelope) = round_up((size(txEnvelopeNoPayload)+TX_BASE_RESULT_SIZE) * minFeeHistorical1KB / 1024)`
...
__open:__ we could consider removing `tx.extendedMetaDataSize` and instead just use `txMaxExtendedMetaDataSizeBytes`.
...
A transaction contains:
* `uint32 tx.extendedMetaDataSize` the maximum size of extended data produced by this transaction
* `int64 tx.flatResourcesFee` fee shared by all “flat fee” resources

`min_ExtendedMetaData_fee(tx) = round_up(tx.extendedMetaDataSize * minFeeExtendedData1KB / 1024)`

Validity constraints:
* per transaction
  * `tx.extendedMetaDataSize <= txMaxExtendedMetaDataSizeBytes`
...
Note that transactions can sill be invalidated more than a ledger in the future, as a consequence validators may apply a certain level of "padding" when computing the minimum fee required before flooding those transactions.
...
Transaction may contain a `ephemeralPayload` (Hash + size), that gets cleared before applying transactions (used in the context of proofs of availability).
