...
struct ContractGasNetworkSettingsV0
{
    ...
    int64 feeRatePerGasIncrement; // cost of GAS_INCREMENT=10k gas
};
...
with
`
resourcesFee(tx) = Gas_fee(tx) + LedgerDataAccess_fee(tx) + NetworkData_fee(tx)
`
...
`Gas_fee(Tx) = round_up(Tx.gas*feeRatePerGasIncrement/GAS_INCREMENT)`
...
`historical_flat_fee(txEnvelope) = round_up((size(txEnvelopeNoPayload)+TX_BASE_RESULT_SIZE) * feeHistorical1KB / 1024)`
...
`historical_flat_fee(txEnvelope) + extendedMetaData_flat_fee(tx)`
