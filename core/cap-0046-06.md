...
There are two ways to initialize the token contract
...
```rust
// All methods below return the contract ID for the newly created contract.
...
fn create_token_from_asset(asset: Object) -> Result<Object, HostError>;

// Creates an uninitialized token contract after verifying the following -
// Define CREATE_CONTRACT_SEPARATOR =
//     "create_token_from_ed25519(contract: Vec<u8>, salt: u256, key: u256, sig: Vec<u8>)"
// Verifies that sha256(params) was signed by key
//     params = vec![CREATE_SEPARATOR, salt, contract]
//     key = key
//     sig = sig
fn create_token_from_ed25519(salt: Object, key: Object, sig: Object) -> Result<Object, HostError>;

// Creates an uninitialized token contract where the contract ID is derived from the
// contract ID that calls this host function and the salt that's passed in.
fn create_token_from_contract(salt: Object) -> Result<Object, HostError>;

// Create an uninitialized token contract using the Stellar source account and salt
// as input to the contract ID that gets created.
fn create_token_from_source_account(salt: Object) -> Result<Object, HostError>;
```
...
`ENVELOPE_TYPE_CONTRACT_ID_FROM_ASSET`. Uniqueness does not apply for other
token contracts, so the other `create_token*` methods uses the creator and a
salt as usual.
