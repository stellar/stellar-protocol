...
If we want to simplify these schemes, we could make the invoker signature
optional (saving 96 bytes). The consequence would be that `get_invoker()` could
trap if the invoker is not set.
