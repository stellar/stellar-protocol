...
index 0e7bc842b..2976045dc 100644
...
index 75f39eb4e..b6616cf32 100644
...
+    ChangeTrustAsset line;
     ...
     int64 limit;
@@ -409,6 +434,35 @@ struct SetTrustLineFlagsOp
...
@@ -476,6 +534,15 @@ case ENVELOPE_TYPE_OP_ID:
...
+    } revokeId;
 ...
 enum MemoType
@@ -635,7 +702,28 @@ struct TransactionSignaturePayload
...
+
 ...
 {
     // emitted to identify the offer
@@ -651,6 +739,32 @@ struct ClaimOfferAtom
     int64 amountBought;
 };
...
@@ -745,7 +859,7 @@ union PathPaymentStrictReceiveResult switch (
...
@@ -789,7 +903,7 @@ union PathPaymentStrictSendResult switch (PathPaymentStrictSendResultCode code)
...
@@ -933,7 +1047,10 @@ enum ChangeTrustResultCode
...
asset in the pool (ordered by the less-than relation among assets). The
...
- `line.liquidityPool().constantProduct().fee != 30`
...
- If pool share trust line does not exist (and therefore needs to be created)
  ...
  - `poolSharesTrustLineCount` is incremented on the
  corresponding liquidity pool and `liquidityPoolUseCount` is incremented
  on each asset trust line. 
  
- If pool share trust line is being deleted
  - `poolSharesTrustLineCount` is decremented on the 
  corresponding liquidity pool, and `liquidityPoolUseCount` is decremented
  on each asset trust line.
  - If `poolSharesTrustLineCount` on the corresponding liquidity pool becomes 0, then that
...
x = ceil[Xy / (Y - y) / (1 - F)] .
```
In this proposal, `F = 0.003` which corresponds to 0.3%.
