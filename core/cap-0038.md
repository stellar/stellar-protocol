...
### SetTrustlineFlagsOp uses ExpandedTrustLineAsset
`SetTrustlineFlagsOp` uses `ExpandedTrustLineAsset` instead of `TrustLineAsset` 
because the former has the full pool key instead of the hash of it. This
allows the issuers to be checked against the source account during validation
