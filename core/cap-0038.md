...
index 0e7bc842..5c9cfd8a 100644
...
+    CLAIMABLE_BALANCE_ID_TYPE_V0 = 0,
+    CLAIMABLE_BALANCE_ID_TYPE_V1 = 1 // used when revoking auth from assets in pools
 };
 ...
 case CLAIMABLE_BALANCE_ID_TYPE_V0:
     Hash v0;
+case CLAIMABLE_BALANCE_ID_TYPE_V1:
+    Hash v1;
 };
...
index 75f39eb4..b5f8476a 100644
...
@@ -635,7 +687,28 @@ struct TransactionSignaturePayload
...
+
 ...
 {
     // emitted to identify the offer
@@ -651,6 +724,32 @@ struct ClaimOfferAtom
     int64 amountBought;
 };
...
@@ -745,7 +844,7 @@ union PathPaymentStrictReceiveResult switch (
...
@@ -789,7 +888,7 @@ union PathPaymentStrictSendResult switch (PathPaymentStrictSendResultCode code)
...
@@ -933,7 +1032,9 @@ enum ChangeTrustResultCode
...
if isAuthorizedToMaintainLiabilities(tl) && !isAuthorizedToMaintainLiabilities(expectedFlag)
    ...
    foreach (poolTL in poolTLs)
        ...
        foreach (assetAndAmount in assetAndAmounts)
            ...
            cb.balanceID.type(CLAIMABLE_BALANCE_ID_TYPE_V1)
            cb.balanceID.v1 = sha256(txSourceAccount, txSourceSeqNum, opIndex, poolTL.liquidityPoolID, assetAndAmount.asset)
...
offers regardless of their authorization state, so it makes sense to treat pool
shares the same.  A deauthorized account can withdraw from a pool, but will not
be able to do anything else with those funds since operations like payments
check authorization.
...
This proposal introduces `TrustLineEntryExtensionV2`, which keeps track of the
number of pool trust lines an asset trust line is involved in. The extension is
used to make sure the trust line is not deleted while corresponding pool trust
lines exist. This is required because without the asset trust line, there is no
way to deauthorize the trust line and force a redeem of the related pool
shares.
