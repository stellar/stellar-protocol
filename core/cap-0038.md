...
The current proposal uses the authorization of individual pool trust lines to 
force a redeem of pool shares. There is an alternative to this solution. We could instead
treat pool shares similar to offers, and force all pool shares for a given asset trust line
to be redeemed when that asset trust line is deauthorized. This approach has a couple different
requirements.

1. The asset trust line must exist as long as an associated pool trust line exists. A new extension
to trust lines will be added, along with new error results for `ChangeTrustOp`. 
```
...
2. We need an efficient way to query for all pool trust lines and pool entries
for a given account and asset.


The proposal doesn't use this approach since it is more complex, but it has the
advantage of being simpler for the end user.
