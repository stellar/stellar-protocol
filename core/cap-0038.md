...
### XDR changes
This patch of XDR changes is based on the XDR files in commit (`ff56e7f207b918aae53710abcc001511ead7ece9`) of stellar-core.

```diff
diff --git a/src/xdr/Stellar-ledger-entries.x b/src/xdr/Stellar-ledger-entries.x
index 26ff33d4..a85a8066 100644
...
@@ -90,7 +91,8 @@ enum LedgerEntryType
     TRUSTLINE = 1,
     OFFER = 2,
     DATA = 3,
-    CLAIMABLE_BALANCE = 4
...
+    LIQUIDITY_POOL = 5
 ...
 struct Signer
@@ -214,12 +216,53 @@ const MASK_TRUSTLINE_FLAGS = 1;
...
@@ -403,6 +446,19 @@ struct ClaimableBalanceEntry
     ext;
 };
...
@@ -479,6 +537,9 @@ case CLAIMABLE_BALANCE:
     {
...
index 70f5ef7d..974b34e3 100644
...
+    CLAWBACK_CLAIMABLE_BALANCE = 20,
+    // SET_TRUST_LINE_FLAGS = 21 is in progress for protocol version 16
+    LIQUIDITY_POOL_DEPOSIT = 22,
+    LIQUIDITY_POOL_WITHDRAW = 23
 };
 
 /* CreateAccount
@@ -220,7 +223,7 @@ struct SetOptionsOp
 */
...
+    TrustLineAsset line;
     ...
     int64 limit;
@@ -390,6 +393,47 @@ struct ClawbackClaimableBalanceOp
     ClaimableBalanceID balanceID;
 };
...
+*/
+union LiquidityPoolDepositOp switch (LiquidityPoolType type)
+{
+case LIQUIDITY_POOL_CONSTANT_PRODUCT:
+    struct
+    {
+        Asset assetA;     // assetA < assetB
+        Asset assetB;
+        int64 amountA;    // amount of A to deposit if
+                          // amountA/amountB <= reserveA/reserveB
+        int64 amountB;    // amount of B to deposit if
+                          // amountA/amountB >= reserveA/reserveB
+        int64 minAmountA; // minimum amount of A when depositing specified
+                          // amount of B; bounds the price
+        int64 minAmountB; // minimum amount of B when depositing specified
+                          // amount of A; bounds the price
+    } constantProduct;
+};
+
...
+*/
+union LiquidityPoolWithdrawOp switch (LiquidityPoolType type)
+{
+case LIQUIDITY_POOL_CONSTANT_PRODUCT:
+    struct
+    {
...
@@ -614,7 +662,28 @@ struct TransactionSignaturePayload
...
+
 ...
 {
     // emitted to identify the offer
@@ -630,6 +699,26 @@ struct ClaimOfferAtom
     int64 amountBought;
 };
...
@@ -724,7 +813,7 @@ union PathPaymentStrictReceiveResult switch (
...
@@ -768,7 +857,7 @@ union PathPaymentStrictSendResult switch (PathPaymentStrictSendResultCode code)
...
- `lpdo.constantProduct().assetA` is invalid
- `lpdo.constantProduct().assetB` is invalid
- `lpdo.constantProduct().assetA >= lpdo.constantProduct().assetB`
- `lpdo.constantProduct().amountA <= 0`
- `lpdo.constantProduct().minAmountB <= 0`
- `lpdo.constantProduct().maxAmountB < lpdo.constantProduct().minAmountB`
