...
and use them exclusively by 2030 ([CNSA 2.0]); X.509/PKI algorithm
identifiers are standardized ([RFC 9881]); and support has shipped in
general-purpose cryptographic libraries ([OpenSSL 3.5]), language runtimes
(JDK 24, [JEP 497]) and cloud key-management services ([AWS KMS ML-DSA]).
...
The `min_supported_protocol` value shown is illustrative and will be set to
the protocol version this CAP is released in.
...
products respectively) and differs by roughly 2–3x between ML-DSA-44 and
ML-DSA-87; this two-dimensional cost structure cannot be expressed by a
single linear cost model. Final calibration parameters are TBD; see
...
[Motivation](#motivation)), which is what matters for the custom-account
use case: contracts verify what signers can produce.
...
- **No composability demand.** Unlike the BLS12-381 functions, from which
  many distinct protocols are built, ML-DSA verification is a single fixed algorithm; there
  is no established ecosystem of protocols composed from its internals.
...
- **Boundary data movement.** Composing a verification from primitives
  ...
  verification) across the wasm boundary repeatedly, with each crossing
  metered and copied; a one-call interface keeps all intermediate state in
  the host.
- **Metering surface.** Every exposed primitive becomes an individually calibrated operation maintained
  indefinitely. A single verification function per parameter set minimizes
  this surface.
...
The pre-hashed variant (HashML-DSA, FIPS 204 §5.4) is not included: it
exists to accommodate signing flows that cannot stream the full message,
offers strictly weaker domain separation, and can be added later as separate
functions if signer ecosystems converge on it.

Unlike `verify_sig_ecdsa_secp256r1`, which takes a 32-byte message digest,
these functions take the full message. This is not a free choice: ML-DSA
binds the verifying key into the message representative
(`mu = H(H(pk) || M')`) inside the algorithm, so a conforming verification
cannot accept an externally pre-hashed input. The message length is
accounted for by the linear term of the verification cost types.
