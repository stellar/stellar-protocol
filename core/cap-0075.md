...
- `t`: State size. The internal state contains `t` field elements. Common values are 2, 3, or 4 depending on the use case (e.g., binary tree hashing uses t=3). Poseidon does not restrict the state size. Poseidon2 supports state sizes of 2, 3, 4, 8, 12, 16, 20, or 24.

- `d`: S-box degree. The S-box is defined as S(x) = x^d. Only d=5 is supported, which is the standard choice for both BLS12-381 and BN254.
...
-  (Poseidon2 only) `t` is not one of 2, 3, 4, 8, 12, 16, 20, or 24
- `d` is not 5
- `rounds_f` is not even
...
The new cost types `Bn254FrToU256`, `Bn254FrAddSub`, `Bn254FrMul`, `Bn254FrPow`, and `Bn254FrInv`, together with the existing `Bn254FrFromU256`, meter BN254 scalar field operations. For BLS12-381 operations, existing cost types from CAP-0059 are used.
...
This CAP does not introduce backward incompatibilities. The new host functions are only available in protocol version 25 and later.
...
The host function performs minimal parameter validation (vector lengths, supported S-box degree and state sizes). Scalar inputs are not range-checked: `U256Val` values greater than or equal to the field order are reduced modulo `r`. It does not validate whether the provided parameters (rounds, matrices, constants) constitute a secure configuration. Users are responsible for selecting cryptographically sound parameters.
