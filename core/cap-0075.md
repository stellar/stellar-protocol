...
- `mds`: A `t`-by-`t` MDS matrix represented as `Vec<Vec<Scalar>>`. The matrix
...
- `mat_internal_diag_m_1`: Internal matrix diagonal minus 1, represented as
  `Vec<Scalar>` of length `t`. Poseidon2 replaces the MDS matrix in partial
...
- `round_constants`: A matrix of size `(rounds_f + rounds_p)` by `t`,
  represented as `Vec<Vec<Scalar>>`. Each round adds a different constant to
  each state element.
