...
This design enables the same core T-REX interface to work with vastly different
regulatory and technical requirements for identity verification, such as Merkle
trees, Zero-Knowledge proofs, claim-based systems, and others. Furthermore, the
modular hook-based system supports diverse regulatory requirements through
pluggable compliance rules.

### Compliance Hook Design

Two properties of Soroban's execution model shape how the token communicates
with compliance modules, and both move this design away from the EVM-based
ERC-3643 it adapts. They are worth understanding before reading the hook
interface specified later in this document.

**1. One hook per operation, instead of a validate-then-act pair.**

ERC-3643 follows a validate-then-act pattern: a read-only pre-check
(`canTransfer`) consulted _before_ tokens move, and a separate state-update
call (`transferred`) made _after_.

On Soroban, **atomicity** makes that separate pre-check redundant for
enforcement. Any panic anywhere in a call tree reverts the _entire_ transaction
and leaves no partial state behind, so a module can reject an operation by
panicking from a hook that runs as part of that operation (after its balance
changes, but in the same transaction). The result is the same as blocking it up
front: nothing the operation did survives the revert. A dedicated read-only
validation hook would add nothing on top of a single hook that both checks and
panics.

This standard therefore uses **one hook per operation** (`transferred`,
`created`, `destroyed`). Each hook enforces policy by panicking to reject, and
records whatever state the module needs. This keeps the design simpler and
removes a class of potential misconfigurations.

**2. All relevant token state is pushed to modules, to keep the interface
future-proof.**

Soroban forbids reentrancy: while a hook is executing, the calling token
contract is still on the call stack, so a module **cannot call back into the
token** to read state such as a balance or a frozen amount. Everything a module
needs to reach its decision must be handed to it at call time.

This may create a versioning hazard. The day a new kind of compliance module
needs a piece of token-held state that the hooks do not already carry,
supplying it would be a breaking change to the signature every existing module
implements. To avoid recurring breaking changes, the token **relays the
relevant account state to modules proactively**, compacted into structs rather
than an ever-growing list of parameters. Each account is passed as an
`AccountSnapshot` bundling its address, balance, and frozen amount, and the
transfer hook additionally carries a `TransferKind` describing how the transfer
was initiated. The concrete shapes are defined in the
[Compliance Trait](#the-compliance-trait) section.
...
The T-REX token does not implement compliance or identity logic itself. It
delegates both to external contracts, a Compliance contract and an Identity
Verifier contract, and calls into them at fixed points during transfers,
minting, burning, and recovery. The token depends on these contracts only
through a fixed set of function signatures.

The standard pins down the _interface_ and leaves the _behavior_ to the
contract sitting behind that interface. Any Compliance or Identity Verifier
contract can be plugged into a token deployment.

From the token's side, that required surface is small. A **Compliance**
...
The token calls `transferred` after every transfer (standard, delegated, or
forced), `created` after every mint, and `destroyed` after every burn. Each one
runs once the operation's balance changes are already applied, but still inside
the same transaction. A hook enforces policy by panicking, which reverts the
whole transaction atomically, and otherwise records whatever state the module
needs. With no modules registered these hooks do nothing, so a token without
compliance modules behaves as if every operation is allowed.

An **Identity Verifier** contract must expose:
...
The token calls `verify_identity` for every account that sends or receives
tokens, and that call panics when the account is not verified.

The token calls `recovery_target` during `recover_balance`, to confirm that the
new wallet is the registered recovery target for the lost one before moving any
balance.

Defining the interfaces, and delegating them to other contracts rather than
baking them into the token, is what gives the standard its flexibility for
...
4. **Configure Connections**: Link the T-REX token to its compliance and
   identity verifier contracts using `set_compliance()` and
   `set_identity_verifier()`. Additionally, configure internal connections
   within the compliance stack (e.g., linking compliance contract to compliance
   modules) and identity stack (e.g., linking identity verifier to claim
   topics/issuers or custom registries) as needed for your implementation.
...
The transfer event is emitted when tokens are transferred from one address to
another, including forced transfers and recovery operations.
...
The recovery success event is emitted when tokens are successfully recovered
from a lost wallet to a new wallet.
...
pub trait IdentityVerifier {
    ...
    /// Returns the target address for the recovery process for the old
    /// account, or `None` if the old account is not a target of any recovery
    /// process. Consumed by `RWAToken::recover_balance` to validate that an
    /// identity recovery has already been performed before moving balances.
    fn recovery_target(e: &Env, old_account: &Address) -> Option<Address>;
...
/// that triggered the hook. Soroban forbids reentrancy and the token contract
/// is still on the call stack while a hook runs, so a module cannot call back
/// into the token to read a balance; the token passes this snapshot in instead.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct AccountSnapshot {
    pub address: Address,
    pub balance: i128, // total balance, before the operation
    pub frozen: i128,  // partially-frozen portion of `balance`
}

/// Identifies who initiated a transfer and under what authority, so a module
/// can decide whether its policy applies.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TransferKind {
    Standard,           // the holder moves its own tokens
    Delegated(Address), // a delegate moves the holder's tokens; carries the spender
    Forced,             // a privileged operation (forced transfer, recovery)
}
...
pub trait Compliance {
    ...
    // Operation hooks - one per token operation, each called after the
    // operation's balance changes are applied but within the same transaction.
    // A module rejects the operation by panicking (reverting the whole
    // transaction) and records any bookkeeping it needs otherwise.
    fn transferred(e: &Env, from: AccountSnapshot, to: AccountSnapshot, amount: i128, kind: TransferKind, token: Address);
    fn created(e: &Env, to: AccountSnapshot, amount: i128, token: Address);
    fn destroyed(e: &Env, from: AccountSnapshot, amount: i128, token: Address);
}
...
Each hook accepts a `token: Address` parameter identifying the calling RWA
token contract. This allows a single compliance contract to serve multiple
...
Each participating account is passed as an `AccountSnapshot` carrying its
pre-operation `balance` and partially-frozen amount. Because Soroban forbids
reentrancy and the token contract is still on the call stack while a hook runs,
a module cannot call back into the token to read these values; the snapshot is
how the token hands them in, enabling rules such as maximum-balance or
holding-period checks without the module mirroring balances itself.

The transfer hook additionally receives a `TransferKind`. Privileged operations
(`forced_transfer`, `recover_balance`) pass `TransferKind::Forced`, letting a
policy module exempt them from its checks (a sanctions or lockup rule should
not block a court-ordered seizure or an account recovery), while a bookkeeping
module still records the movement so its state stays consistent. Standard
transfers pass `TransferKind::Standard`, and delegated (`transfer_from`)
transfers pass `TransferKind::Delegated`, carrying the spender.

**Validity requirements for hooks:**

A hook that mutates per-token state MUST be protected against caller spoofing
of the `token` parameter. Concretely, such implementations MUST:
...
Without both checks, an attacker could call a hook directly with a `token` they
do not control and corrupt that token's per-module state (e.g., transfer
counters, holding-period trackers). A hook that only enforces a policy and
keeps no state of its own (panicking on violation, mutating nothing) needs no
such authorization: an unauthenticated call can at worst fail the caller's own
transaction.
...
pub enum ComplianceHook {
    Transferred,    // After a transfer: enforce policy (panic to reject) and/or update state
    Created,        // After a mint: enforce policy (panic to reject) and/or update state
    Destroyed,      // After a burn: enforce policy (panic to reject) and/or update state
}
```

A module registers only for the hooks of the operations it cares about. A
module that enforces a rule on transfers but keeps no state registers for
`Transferred` alone; a module that tracks supply registers for `Created` and
`Destroyed`; and so on.
...
- Hook: `Transferred`
- Logic: Track cumulative volume per user and panic when a transfer would
  exceed a daily/monthly limit

**Jurisdiction Restrictions Module**:

- Hook: `Transferred` + `Created`
- Logic: Panic when the recipient's jurisdiction is blocked (forced transfers
  exempt via `TransferKind::Forced`)

**Holding Period Module**:

- Hook: `Created` + `Transferred`
- Logic: Record a lock on each mint, and panic on a transfer that would spend
  still-locked tokens

**Investor Accreditation Module**:

- Hook: `Created`
- Logic: Panic when minting to an unaccredited investor

#### Shared Compliance Infrastructure

Compliance contracts are designed to be **shared across multiple T-REX
tokens**, reducing deployment costs and ensuring consistent regulatory
enforcement:
...
2. Balance Recovery: Managed by the T-REX token, transfers tokens from old to
   new account after verifying the identity recovery mapping exists
...
- **Compliance**: The compliance hook still runs, but the transfer is signalled
  as `TransferKind::Forced`. Policy modules are expected to exempt forced
  operations (this is what lets a seizure or recovery proceed past a lockup or
  jurisdiction rule), while modules that track state still update their books.
  This is finer-grained than an outright bypass: whether a forced operation is
  exempt becomes a per-module decision rather than all-or-nothing.
...
- **Operator Authorization**: All administrative functions require `operator`
  authorization
- **Flexible Access Control**: While the T-REX interface itself doesn't
  prescribe a specific access control model, implementations can integrate with
  external access control systems as needed
...
**Country Data Type Flexibility:**

The trait interface uses `Val` for country data parameters instead of a
concrete type, allowing implementors to define their own country data structure
(e.g., the reference `CountryData` with rich jurisdictional modeling, hash
commitments, Merkle commitments, ZK commitments, or off-chain attestations)
while preserving the same contract interface. Implementors handle serialization
between `Val` and their concrete type internally.

An alternative design would use an associated type on the trait (e.g.,
`type CountryData: FromVal<Env, Val>`), but the Soroban SDK does not currently
support deriving `#[contractclient]` for traits with associated types, which
would prevent generating cross-contract clients from the trait definition.
Using `Val` directly avoids this limitation.
...
pub trait IdentityRegistryStorage {
    ...
    ///   (each entry is a `Val` whose concrete shape is defined by the
    ///   implementor).
...
- `v0.2.0` - Add `token: Address` to all compliance hooks (`can_transfer`,
  `can_create`, `transferred`, `created`, `destroyed`); add the
  `get_modules_for_hook` and `is_module_registered` getters; add
  `IdentityVerifier::recovery_target`; replace the `IdentityRegistryStorage`
  `CountryData` associated type with `Vec<Val>` parameters; rename the
  `Recovery` event to `RecoverySuccess`; drop the `operator` topic from
  `AddressFrozen`.
- `v0.3.0` - Merge the compliance validation and state-update hooks. Remove
  `can_transfer` and `can_create` (and the `CanTransfer`/`CanCreate`
  `ComplianceHook` variants), so each token operation has a single hook
  (`transferred`, `created`, `destroyed`) that both enforces policy (by
  panicking to reject, which reverts the whole operation atomically) and
  updates state, running after the operation's balance changes within the same
  transaction. Pass each account participant as an `AccountSnapshot`
  (`address`, `balance`, `frozen`) instead of a bare `Address`, so modules
  receive pre-operation balances without calling back into the token. Add a
