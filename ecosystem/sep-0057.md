...
(`canTransfer`) consulted *before* tokens move, and a separate state-update call
(`transferred`) made *after*.

On Soroban, **atomicity** makes that separate pre-check redundant for
enforcement. Any panic anywhere in a call tree reverts the *entire* transaction
...
This may create a versioning hazard. The day a new kind of compliance module needs
a piece of token-held state that the hooks do not already carry, supplying it
would be a breaking change to the signature every existing module implements.
To avoid recurring breaking changes, the token **relays the relevant account
state to modules proactively**, compacted into structs rather than an
ever-growing list of parameters. Each account is passed as an
...
was initiated. The
concrete shapes are defined in the [Compliance Trait](#the-compliance-trait)
section.
...
The standard pins down the *interface* and leaves the *behavior* to the contract sitting behind that
interface. Any Compliance or Identity Verifier contract can be plugged into a
token deployment.
...
The token calls
`recovery_target` during `recover_balance`, to confirm that the new wallet is
the registered recovery target for the lost one before moving any balance.

Defining the interfaces, and delegating them to other contracts
rather than baking them into the token, is what gives the standard its
...
A hook that mutates per-token state MUST be protected against caller spoofing of
the `token` parameter. Concretely, such implementations MUST:
...
counters, holding-period trackers). A hook that only enforces a policy and keeps
no state of its own (panicking on violation, mutating nothing) needs no such
authorization: an unauthenticated call can at worst fail the caller's own
transaction.
