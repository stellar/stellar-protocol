...
- A **Home Domain**: a domain hosting a [SEP-1 stellar.toml](sep-0001.md) containing a `WEB_AUTH_FOR_CONTRACTS_ENDPOINT` (URL), `WEB_AUTH_CONTRACT_ID`, and `SIGNING_KEY` (`G...`).
...
1. The **Client** looks up the `WEB_AUTH_FOR_CONTRACTS_ENDPOINT`, `WEB_AUTH_CONTRACT_ID` and `SIGNING_KEY` (i.e. **Server Account**) from the `stellar.toml`.
...
`WEB_AUTH_FOR_CONTRACTS_ENDPOINT` in their [`stellar.toml`](sep-0001.md) file. This is how a wallet knows where to find the
...
POST request to `WEB_AUTH_FOR_CONTRACTS_ENDPOINT` using one of the following formats (both should be equally supported by the
server):
