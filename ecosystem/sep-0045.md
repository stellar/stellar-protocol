...
- A **Server**: a server providing the `WEB_AUTH_FOR_CONTRACTS_ENDPOINT` that implements the GET and POST operations
  discussed
  - The `SIGNING_KEY` from the **Home Domain** is the **Server Account**. in this document. The server's domain may be
    the **Home Domain**, a sub-domain of the **Home Domain**, or a different domain.
