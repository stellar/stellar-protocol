...
1. The **Client** simulates the transaction with the signed authorization entries and verifies the following to ensure
   ...
   - The transaction's ledger footprint `read_write` set contains only `contract_data` entries where:
     ...
     - (Optional) The `contract` is the `WEB_AUTH_CONTRACT_ID`, and the `key` is `ledger_key_contract_instance`
       if the web authentication contract instance has been archived.
...
- `authorization_entries`: XDR-encoded `SorobanAuthorizationEntries`. It contains an entry for the **Client
...
- `0.1.1`: Handle archived SEP-45 contract instances ([#1827](https://github.com/stellar/stellar-protocol/pull/1827))
- `0.1.0`: Initial draft
