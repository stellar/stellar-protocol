...
1. The **Client** obtains a signature from the **Client Domain Account** for the authorization entry where
   ...
   request
1. If the **Client Account** requires additional authorization entries (`require_auth`s in its `__check_auth`), the **Client** must construct and add these entries to the authorization entries array and ensure they are signed by their respective addresses
...
1. The **Server** verifies that the `contract_address` in each `web_auth_verify` authorization entry matches the `WEB_AUTH_CONTRACT_ID` from the **Server**'s `stellar.toml`
1. The **Server** verifies that the standard (client, server, optional client domain) authorization entries have `function_name` of `web_auth_verify`
1. The **Server** verifies that the `args` map in each `web_auth_verify` authorization entry match the expected values and are the same across all `web_auth_verify` authorization entries:
   1. The `account` value matches the **Client Account** address
   1. The `home_domain` value matches the **Home Domain**
   1. The `web_auth_domain` value matches the **Server**'s domain
   1. The `web_auth_domain_account` value matches the **Server Account**
   1. The `client_domain_account` value matches the **Client Domain Account** if the **Client** included a `client_domain` in the request, otherwise it is not present
1. The **Server** verifies that the `nonce` argument is the same across all `web_auth_verify` authorization entries and is unique
...
- `0.2.0`: Handle `require_auth` in `__check_auth` ([#1648](https://github.com/stellar/stellar-protocol/pull/1648))
- `0.1.0`: Initial draft
