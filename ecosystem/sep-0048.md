...
Tooling and SDKs must be able to generate client code for calling contracts and parsing its events.
...
An event spec entry describes an event that may be published by the
contract.
...
The data value of the event is structured according to the `SCSpecEventDataFormat`.

- `SINGLE_VALUE`: The data value contains a single `SCVal` of any type. The event should have a single event parameter that is located in data. If the event has no event parameters located in data the value should be an `SCV_VOID`.
- `VEC`: The data value contains an `SCV_VEC` with data parameters in declaration order.
- `MAP`: The data value contains an `SCV_MAP` with each data parameter mapping to a map entry with data parameter names as `SCV_SYMBOL` the key and their values as the map entry value.
