...
A standard for contracts to self-describe their exported interface including functions, events, and user-defined types.
...
the functions and events exported by a contract. Tooling and SDKs must be able
to generate client code for calling contracts and interpreting their events.
Off-chain systems must be able to present a human friendly interface describing
a contract's callable interface and observable events.

In addition to callable functions, many smart contracts emit events to signal
that a specific action has occurred. For off-chain clients, indexers, and user
interfaces to effectively monitor and interpret these on-chain signals, a
standardized format for describing the structure of events is necessary.
...
This SEP defines a format for communicating about a contract's interface,
including its functions, events, and user-defined types, as well as a common
location to store the contract interface inside the Wasm files.
...
An event spec entry describes a contract event that can be emitted by the contract.

The event name must match the first topic emitted when the event is published.
The event name is stored as a symbol in `topics[0]` of the `ContractEvent`.

The `params` field is a list of the event's parameters, which can be located
either in the event's topic list or in the event's data payload.

The `dataFormat` field specifies how parameters marked with location `DATA` are
encoded in the event's data payload:
- `SINGLE_VALUE`: The data contains a single `SCVal`.
- `VEC`: The data contains an `SCVec` with parameters in declaration order.  
- `MAP`: The data contains an `SCMap` with parameter names as keys.
...
topic list (`TOPIC_LIST`) or data payload (`DATA`).
...
#[contractevent(data_format = "single_value")]
...
{
  "event_v0": {
    "doc": "Transfer event emitted when tokens are transferred.",
    "lib": "",
    "name": "transfer",
    "params": [
      {
