...
the functions exported by a contract and the shape of its events published.
Tooling and SDKs must be able to generate client code for calling contracts and
parsing its events. Off-chain systems must be able to present a human friendly
interface describing a contract interface.
...
The `prefixTopics` field contains a list of static values that become the first
topic(s) of the event. The static values are defined in the spec as `SCSymbol`,
and when identifying published events may be of the `SCV_SYMBOL` or
`SCV_STRING` `SCVal` type.

The `params` field is a list of the event's parameters, which are dynamic
values that will be included in the published event. Event parameters are
located either in the event's topic list or in the event's data payload. Event
parameters located in the event's topic list appear after the `prefixTopics`.
Event parameters located in the event's data are structured in the data
according to the `dataFormat` field.
...
- `SINGLE_VALUE`: The data value contains a single `SCVal` of any type. The
  event should have a single event parameter that is located in data. If the
  event has no event parameters located in data the value should be an
  `SCV_VOID`.
...
And that would map the following publish to the following Contract Event XDR:
...
**The above XDR structures not defined by this proposal are accurate as of
Protocol 23, and may change over time. The example is included for illustrative
purposes at the time of the proposal has been authored.**
