...
And that maps to the following `ContractEvent` XDR:
...
> [!NOTE] The `ContractEvent` XDR structure is not defined by this proposal. It
> is accurate as of Protocol 23, and may change over time. The example is
> included to illustrate how the event parameters should map to the `topics`
> and `data` fields of the `ContractEvent`.
