...
| `buy_asset`            | string                                                                        | The asset the client would like to exchange for `sell_asset`.                                                                                                                                                                                                                                  |
| `buy_amount`           | string                                                                        | The amount of `buy_asset` to be exchanged for `sell_asset`. It could be different from the `buy_amount` provided in the request, depending on how fees are applied by the Anchor. `total_price * buy_amount = sell_amount` must be true up to the number of decimals required for `buy_asset`. |
...
| `buy_asset`            | string                                                                        | The asset the client would like to exchange for `sell_asset`.                                                                                                                                           |
| `buy_amount`           | string                                                                        | The amount of `buy_asset` to be exchanged for `sell_asset`. `total_price * buy_amount = sell_amount` must be true up to the number of decimals required for `buy_asset`.                                |
...
- `v2.5.1`: Correct the `buy_amount` field description to use `total_price`
  instead of `price`, matching the Price Formulas section, the worked examples,
  and the reference implementation.
  ([#2003](https://github.com/stellar/stellar-protocol/pull/2003))
