...
This protocol enables anchors to accept off-chain assets in exchange for different on-chain assets, and vice versa. Specifically, it enables anchors to provide quotes that can be referenced within the context of other existing Stellar Ecosystem Proposals, like [SEP-6] and [SEP-31], for instance. How the exchange of assets is facilitated is outside the scope of this document.
...
All endpoints accept requests with the following `Content-Type`:
- `application/json`
...
The prices returned include any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, amount, delivery method, country code, or other factors. Anchors can optionally detail the embedded costs of the margin in the response field `price_details`.
...
`sell_amount` | string | The amount of `sell_asset` the client would exchange for each of the `buy_assets` available.
...
`price_details` | array | (optional) An array of objects describing the fees, taxes or other costs that were used to calculate the conversion price. This can be used to datail the price components for the end user.

Object Schema for `price_details`.

Name | Type | Description
-----|------|------------
`name` | string | The name of the fee or tax.
...
`value` | string | The amount of asset applied.
...
The price returned includes any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, amount, delivery method, country code, or other factors. Anchors can optionally detail the embedded costs of the margin in the response field `price_details`.
...
Name | Type | Description
-----|------|------------
`sell_asset` | string | The asset the client would like to sell. Ex. `stellar:USDC:G...`, `iso4217:ARS`
...
The quote returned includes any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, the delivery method, amount, country code, or other factors. Anchors can optionally detail the embedded costs of the margin in the response field `price_details`.
...
This endpoint can be used to fetch a previously-provided firm quote. Quotes referenced in other protocols must be available at this endpoint even past the `expires_at` expiration for the quote.
...
* `v1.7.0`: Add optional field `price_details` to the response body of the prices and quotes endpoints. ([#1199](https://github.com/stellar/stellar-protocol/pull/1199))
