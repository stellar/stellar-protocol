...
* `v1.7.0`: Add `context` to the `POST /quote` and `GET /prices` request parameters, so anchors can calculate the quote taking into consideration what they will be used for. Also, add optional field `fee_details` to these endpoints response body. ([#????](https://github.com/stellar/stellar-protocol/pull/????))
