...
* `v1.7.0`: Add request parameter `context` to the `POST /quote` and `GET /prices` request parameters, so anchors can calculate the quote taking into consideration what they will be used for. ([#????](https://github.com/stellar/stellar-protocol/pull/????))
