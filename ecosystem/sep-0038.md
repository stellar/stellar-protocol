...
Name | Type | Description
-----|------|------------
`effective_price` | string | The effective conversion price offered by the anchor for one unit of `buy_asset` in terms of `sell_asset`, including fees. In traditional finance, `buy_asset` would be referred to as the base asset and `sell_asset` as the counter asset.
...
To better understand the relation between the `*price`, `*amount` and `fee` fields, please refer to the [Price Formulas](#price-formulas) section.
...
```js
sell_amount = effective_price * buy_amount
```
