...
The prices returned include any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, amount, delivery method, country code, or other factors. Anchors can optionally detail the costs of the margin in the response field `fee_details`.
...
`context` | string | The context for what this quote will be used for, so the anchor can calculate the price accordingly. Must be one of `sep6` or `sep31`.
...
`fee_details` | array | (optional) An array of objects describing the fees that were used to calculate the conversion price. This can be used to datail the price components for the end user.

Object Schema for `fee_details`.

Name | Type | Description
-----|------|------------
`name` | string | The name of the fee.
`description` | string | (optional) A text describing the fee.
`asset` | string | The asset in which the fee is applied, represented through the [Asset Identification Format](#asset-identification-format). All fees under `fee_details` must be in the same asset.
...
The price returned includes any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, amount, delivery method, country code, or other factors. Anchors can optionally detail the costs of the margin in the response field `fee_details`.
...
The quote returned includes any margin that the provider may keep as a service fee, and this margin may vary depending on the directional flow of funds, the delivery method, amount, country code, or other factors. Anchors can optionally detail the costs of the margin in the response field `fee_details`.
...
* `v1.7.0`: Add `context` to the `POST /quote` and `GET /prices` request parameters, so anchors can calculate the quote taking into consideration what they will be used for. Also, add optional field `fee_details` to these endpoints response body. ([#1204](https://github.com/stellar/stellar-protocol/pull/1204))
