...
Name | Type | Description
-----|------|------------
`price` | string | The *effective* price (including fees) offered by the anchor for one unit of `buy_asset` in terms of `sell_asset`. In traditional finance, `buy_asset` would be referred to as the base asset and `sell_asset` as the counter asset.
`nominal_price` | string | The nominal price offered by the anchor for one unit of `buy_asset` in terms of `sell_asset`, without including any fees.
...
{
  "price": "5.42",
  "nominal_price": "5.29", // (500-12)/92.25
  "sell_amount": "500",
  "buy_amount": "92.25",
  "fee": {
    "total": "12.00",
...
{
  "price": "0.18",
  "nominal_price": "0.178", // 100/(555.56+6)
  "sell_amount": "100",
  "buy_amount": "555.56",
  "fee": {
    "total": "6.00",
...
The following `price` formula should always hold true:

```js
price = response.sell_amount / response.buy_amount
```

The relation between the request and response amounts as well as the price and nominal price can vary acording with the Anchor's buseness logic.

Here is an example where the Anchor uses inclusive fees and the request is made in terms of `sell_asset`:

```js
request.sell_asset = USDC  // request defined in terms of sell_asset
request.sell_amount = 100 = sell_amount
sell_asset = USDC
buy_asset = BRL
fee = 10 USDC
nominal_price = 1/5
...
Here is an example where the Anchor uses additive fees and the request is made in terms of `buy_asset`:

```js
request.buy_asset = BRL  // request defined in terms of buy_asset
request.buy_amount = 500 = buy_amount
buy_asset = BRL
sell_asset = USDC
fee = 10 USDC
nominal_price = 1/5
...
The Wallet should always rely on the send/receive amounts returned from the response, rather than the ones sent in the request.
...
```json
{
  ...
  "price": "5.42",
  "nominal_price": "5.25", // (500-24.99)/92.25 
