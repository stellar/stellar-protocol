...
This protocol's endpoints use authentication in the form of a [SEP-10](sep-0010.md)/[SEP-45](sep-0045.md) JSON Web Token
...
Optional [`SEP-10`]/[`SEP-45`]. If authentication is provided it can be used to personalize the response.
...
Get the amount and asset available for selling `100` of the `stellar:USDC:GA5...ZVN"` with the `BRA` country code and
the `ACH` delivery.
...
Get the amount and asset available for selling `500` of the `iso4217:BRL` with the `BRA` country code and the `PIX`
delivery method.
...
| `fee`         | object | An object describing the fee used to calculate the conversion price. This can be used to detail the price components for the end-user.                                                                                                     |
...
submit a Stellar transaction delivering funds to the anchor before the expiration included in the response, paying a fee
as determined by state of the Stellar network.
...
- `v2.5.0`: Add support for SEP-45 authentication. ([#1666](https://github.com/stellar/stellar-protocol/pull/1666))
- `v2.4.1`: `sell_delivery_methods` and `buy_delivery_methods` should match corresponding `funding_methods` options
  specified in the SEP-6 or SEP-31 protocol. ([#1567](https://github.com/stellar/stellar-protocol/pull/1567/))
- `v2.4.0`: Add `sell_delivery_method` and `buy_delivery_method` to the `GET /quote` and `POST /quote` response.
