...
Anchors will no longer have to wait until the user's Stellar account has trust line to the sending asset before sending funds. Instead, anchors can make the payment using a ClaimableBalance operation and the user's Stellar account can claim the funds at their own convenience.
...
* Accept the `claimable_balance_supported` field in `POST /transactions/deposit/interactive` requests and submit deposit transactions using Claimable Balance operations to Stellar accounts that don't yet have a trust-line to the asset requested.
* Add `claimable_balance_id` to their [deposit transactions responses](####Fields-for-deposit-transactions)

**Anchors and Wallets**: Both anchors and wallets must also support the aforementioned ["stellar account doen't have a trust line for that asset."](#stellar-account-doesnt-trust-asset) flow.
...
Polling allows the wallet to detect claimable balances and notify their users that their is a balance available to claim. See the [Claim Account Claimable Balance](#Wallet Feature: Claim Account Claimable Balance) section to learn more about how to claim a balance.
...
1. Anchors detect that the user's Stellar account doesn't have a trust-line.
1. The anchor should then report transaction status as `pending_anchor` and submit a Stellar transaction containing a create claimable balance operation
