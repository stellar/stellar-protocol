...
[Claimable Balances](https://www.stellar.org/blog/protocol-14-improvements) allow for payments to accounts that do not have a trustline for the asset sent.

Anchors can optionally use this functionality to send SEP-24 deposit transactions to Stellar accounts that don't yet have a trustline to the asset they have requested.

Wallets should poll for their users' received Claimable Balances so that users can "claim" them. Claiming a Claimable Balance adds a trustline to the account for the asset claimed and adds the amount transacted.

1. If the given Stellar `account` does not exist, on receipt of the deposit, the
   anchor should use `create_account` and/or the given Stellar `account` does
   not have a trustline to the asset's issuer account.
2. The `Anchor` proceeds with the deposit flow via the
   [CAP-0023](../core/cap-0023.md) Create Claimable Balance operation.
