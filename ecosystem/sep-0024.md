...
`memo` | string | (**deprecated**, optional) This field was originally intended to differentiate users of the same Stellar account. However, the anchor should use the `memo` and `memo_type` values included in the decoded SEP-10 JWT instead. See the [Shared Account Authentication](#shared-accounts) section for more information.
