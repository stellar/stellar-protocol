...
`memo` | string | (**deprecated**, optional) This field was originally intended to differentiate users of the same Stellar account. However, the anchor should use the memo value included in the decoded SEP-10 JWT's `sub` attribute instead. Anchors should still support this parameter to maintain support for outdated clients. See the [Shared Account Authentication](#shared-accounts) section for more information.
