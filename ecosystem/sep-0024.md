...
- An anchor must define the location of their `TRANSFER_SERVER_SEP0024` in
  ...
  the anchor's server.
- Anchors and clients must support [SEP-10](sep-0010.md) web authentication to
...
As stated, Anchors must support [SEP-10](sep-0010.md) web authentication to
enable authenticated deposits, withdrawals, or transaction history lookups.
Clients must submit the JWT previously obtained from the anchor via the
[SEP-10](sep-0010.md) authentication flow to all API endpoints that provide
...
- **Authentication**
  - Perform [authentication](#authentication) via SEP-10 before hitting those
    endpoints
...
- **For both deposit and withdrawal**:
  ...
  - Also include the `id` field in the popup URL you provide to the wallet.
    ...
    URL.
  - We recommend you use [SEP-10](#authentication) for authentication in the
  ...
  - Test your interactive flows on mobile. They should be easy to use on all
    devices: make them responsive, handle auto-fill well, and do smart keyboard
