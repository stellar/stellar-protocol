...
This feature provides less friction for Stellar accounts who wish to deposit external assets with an anchor without making a trust line to the anchor's tokenized asset beforehand.
...
* Accept the `claimable_balance_supported` request parameter in `POST /transactions/deposit/interactive` requests and submit deposit transactions using Claimable Balance operations to Stellar accounts that don't yet have a trust-line to the asset requested.
...
1. Make a request to `/transactions/deposit/interactive` and provide the `claimable_balance_supported` request parameter.
2. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
3. The anchor will ignore the `claimable_balance_supported` field, discover the account does not have a trust line, and report transaction status as `pending_trust`.
4. At this point both the wallet and anchor are waiting on the trust line to be established in order to continue the deposit payment. This is described here [steps 4-6](#stellar-account-doesnt-trust-asset)
...
Accounts will still need to establish trust line to the asset before claiming the outstanding balance.
...
3. Anchors detect that the user's Stellar account doesn't have a trust-line.
