...
This feature splits a payment into two separate parts: the creation of a balance, and the claiming of a balance.
...
The flow the user deposits external assets to the anchor's off-chain account and does not have a trustline to the anchor's stellar asset.
...
3. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
4. If the anchor doesnt support Claimable Balance, the anchor's callback or `/transaction` endpoint will indicate that the status is `pending_trust`. If status is `pending_trust` follow the special case [above](#stellar-account-doesnt-trust-asset).
5. Wallet detects that The `claimable_balance_id` value is populated from anchor's `/transaction` endpoint and set transaction status as `completed`.
...
It's recommended that Wallets provides a UI to enable accounts to claim their balances [polled](#wallet-feature-claimable-balance-polling). Accounts will still need to establish trustline to the asset before claiming the outstanding balance.
**NOTE:** The account will only need to establish a trustline to the asset they wish to claim once before claiming.

Example on how to claim a balance. Omitted from the example is setting trust operation.
...
2. Anchors update their internal database record of the transaction to indicate the wallet supports receiving a [claimable balance](#deposit)
3. User sends external assets to the anchor's off-chain account.
4. Anchors detect that the user's Stellar account doesn't have a trustline.
5. From there the anchor will continue the [deposit flow](#basic-anchor-implementation), crafting the transaction as a claimable balance operation.
...
Predicates are one of the [Claimable balance parameters](https://developers.stellar.org/docs/glossary/claimable-balance/#parameters) used when craftting a Claimable Balance transaction. They're used as conditions to meet in order for the recipient to claim their balance.
