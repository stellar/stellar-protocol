...
```
SEP: 0024
Title: Interactive Anchor/Wallet Interface
Author: SDF
Status: Active
Created: 2019-09-18
Updated: 2020-03-28
...
This SEP defines the standard browser/UI implementation for anchors and wallets to interact on behalf of users. It provides normalized interface specifications that allow wallets and other services to interact with anchors using graphical user interfaces. 

While this document specifies an **interactive interface** between wallets and anchors using frontend UIs that collect complex KYC information potentially required by anchors, [SEP-0006](sep-0006.md) provides a **non-interactive interface** based on REST based backend APIs without any browser dependencies. Both SEPs achieve the same thing in different ways and can be implemented alongside each other as as standalone interface.  
