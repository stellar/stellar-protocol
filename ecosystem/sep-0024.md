...
[Protocol 14/15](https://www.stellar.org/blog/protocol-14-improvements?locale=en) now allows anchors to
complete payments using the [ClaimableBalance functionality](../core/cap-0023.md) and the user can collect the funds at any time.

This is an optional, yet recommended, feature that will no longer need to wait
for the `Wallet` or the user to establish a
trust line for the `Account` for the given asset.
