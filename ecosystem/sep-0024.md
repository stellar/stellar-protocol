...
**Anchors and Wallets**: Both anchors and wallets must also support the aforementioned ["stellar account doen't have a trust line for that asset."](#Stellar-account-doesn't-trust-asset).
...
Claimable Balance Polling allows the wallet to update the claimant that their is a balance available to claim and they may need to establish[trust-line](#stellar-account-doesnt-trust-asset) in order to do so.
...
2. Wallets could decode the `results_xdr` of the transaction submitted
