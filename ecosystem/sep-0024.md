...
If the given Stellar `account` does not exist, on receipt of the deposit, the anchor should use `create_account` to create the account with at least enough XLM for the minimum reserve and a trustline (2.01 XLM is recommended). The anchor should take some of the asset that is sent in to pay for the XLM. The anchor should not list this minimal funding as a fee because the user still receives the value of the XLM in their account. The anchor should detect if the account has been created before returning deposit information, and adjust the `min_amount` in the response to be at least the amount needed to create the account.

Since the anchor doesn't have the user account's secret key, the user must create a trustline to the anchor's asset before the anchor can send the remaining asset tokens to the user's account. The anchor should listen for the user to establish this trustline. Once the trustline is there, the anchor should send the remaining asset tokens to the account in Stellar to complete the deposit.
...
The deposit flow can only be fulfilled if the Stellar `Account` has established a trustline for the given asset. To ensure this is accomplished, when initiating the deposit flow, `Wallet` should check if the `Account` has a trustline for the given asset. If it doesn't:

1. `Wallet` checks if `Account` has enough XLM to create a trustline. If it does, skip to step `4`.
2. If `Account` doesn't have enough XLM, `Wallet` starts listening for transactions to the given `Account`, waiting for it to have enough XLM for a trustline.
3. When asked for a deposit, `Anchor` detects if `Account` has enough XLM to create a trustline. If it doesn't, `Anchor` sends the needed amount of XLM to the `Account` for creating a trustline. `Anchor` then starts listening for trustline creations for that `Account`.
4. `Wallet` detects the arrival of XLM in the `Account`, and establishes a trustline.
5. `Anchor` detects the trustline creation in the `Account`. If the asset is `AUTH_REQUIRED`, `Anchor` approves the new trustline.
6. `Anchor` proceeds with the deposit flow.

##### Claimable Balances

[Claimable Balance](https://www.stellar.org/blog/protocol-14-improvements) is an optional feature for both wallets and anchors; which enable payments to accounts that do not have a trustline for the asset being sent.
The a new ledger entry splits a payment into two separate parts: the creation of a balance, and the claiming of a balance.
A balance can be claimed asynchronously by the designated claimant (user) when they're ready.
This feature provides less friction for Stellar accounts who wish to deposit external assets with an anchor without making a trustline to the anchor's tokenized asset beforehand.
Anchors will no longer have to wait until the user's Stellar account has trustline to the sending asset before sending funds. Instead, anchors can make the payment using a ClaimableBalance operation and the user's Stellar account can claim the funds at their own convenience.

**NOTE** in the *future* the following claimable balance implementations will be mandatory in order to be SEP-0024 interoperable.

**Wallets**: To support claimable balances wallets must

* Send `claimable_balance_supported` request parameter in their POST /transactions/deposit/interactive request.

```json
{
"claimable_balance": true
}
```

* Periodically poll for account's available claimable balances.
* Provide a UI that allows users to claim a claimable balance.

**Anchors**: To support claimable balances anchors must

* Accept the `claimable_balance_supported` request parameter in `POST /transactions/deposit/interactive` requests and submit deposit transactions using Claimable Balance operations to Stellar accounts that don't yet have a trustline to the asset requested.
* Add `claimable_balance_id` to their [deposit transactions responses](####Fields-for-deposit-transactions)

**Anchors and Wallets**: Both anchors and wallets must also support the aforementioned ["stellar account doen't have a trustline for that asset."](#stellar-account-doesnt-trust-asset) flow.
Wallets need to be interoperable with anchors that have not adopted the [claimable balance feature](#Claimable-Balances) and vice versa.

##### Wallet Claimable Balance Flow

Wallets will still follow the [typical deposit flow](#Basic-Wallet-Implementation).
Wallets will provide `claimable_balance_supported` request parameter in their POST parameters for `/transactions/deposit/interactive`.
...
If the anchor we're depositing to **supports** claimable balances.

1. Make a request to `/transactions/deposit/interactive` and provide the `claimable_balance_supported` request parameter.
2. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
3. The anchor will report transaction status as `pending_anchor` then make the payment as a claimable balance operation.
4. After transaction is submitted the anchor will report transaction status as `completed`

If the anchor we're depositing to **doesn't support** claimable balances.

1. Make a request to `/transactions/deposit/interactive` and provide the `claimable_balance_supported` request parameter.
2. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
3. The anchor will ignore the `claimable_balance_supported` field, discover the account does not have a trustline, and report transaction status as `pending_trust`.
4. At this point both the wallet and anchor are waiting on the trustline to be established in order to continue the deposit payment. This is described here [steps 4-6](#stellar-account-doesnt-trust-asset)

##### Wallet Feature: Claimable Balance Polling

Polling allows the wallet to detect claimable balances and provide a UI to users that there is a balance available to claim. See the ["Claim Account Claimable Balance"](#wallet-feature-claim-account-claimable-balance) section to learn more about how to claim a balance.
There are a few options as to how a wallet can poll and surface claimable balance information for their users.
1. The wallet can poll the anchor's [transaction history endpoints](#anchor-feature-extend-transaction-history-endpoints-for-claimable-balances)

2. The wallet can poll Horizon's [`/claimable_balances/`](https://developers.stellar.org/api/resources/claimablebalances/list/) endpoint (or Horizon's [`/claimable_balances/`](https://developers.stellar.org/api/resources/claimablebalances/single/) single endpoint) to pull any available claimable balances for the claimant.

   * The single endpoint needs the `claimable_balance_id` in order to retrieve the claimable balance info.
     * This can be obtained by extracting the `claimable_balance_id` from `results_xdr` of the deposit transaction submitted if the anchor does not extend their [transaction history endpoints](#anchor-feature-extend-transaction-history-endpoints-for-claimable-balances)

##### Wallet Feature: Claim Account Claimable Balance

It's recommended that Wallets provides a UI to enable accounts to claim their balances [polled](#wallet-feature-claimable-balance-polling)

[Example on how to claim a balance](https://developers.stellar.org/docs/glossary/claimable-balance/#claim-claimable-balance)
Accounts will still need to establish trustline to the asset before claiming the outstanding balance.

##### Anchor Claimable Balance Flow

1. Wallets make a request to `/transactions/deposit/interactive` and provide the `claimable_balance_supported` request parameter.
2. Anchors mark the transaction as [`claimable_balance_supported=true`](#deposit)
3. Anchors detect that the user's Stellar account doesn't have a trustline.
4. The anchor should then report transaction status as `pending_anchor` and submit a Stellar transaction containing a create claimable balance operation
5. From there the anchor will continue the [deposit flow](#basic-anchor-implementation)

##### Anchor Feature: Extend Transaction history endpoints For Claimable Balances

Wallets query the [transaction endpoint](#transaction-history) to see and display the status of deposits.
Anchors should extend their transaction(s) endpoint response to include the `claimable_balance_id` that the anchor added to the [transaction field](####Fields-for-deposit-transactions).
This will help wallets to provide UI to showcase that their user's deposit was successful and theres an [available claimable balance for them.](#wallet-feature-claimable-balance-polling)
...
##### Anchor Claimable Balance Claimants and Predicates
...
They're used as conditions to meet in order for the recipient to claim their balance.
      It's recommended to use the `UNCONDITIONAL` predicate to allow the claimant to claim the balance without any barriers.
