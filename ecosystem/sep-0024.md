...
[Claimable Balances](https://www.stellar.org/blog/protocol-14-improvements) enables payments to accounts that do not have a trust line for the asset being sent.
This feature provides less friction for Stellar accounts who wish to make an offchain deposit to anchors without making a trust line before the deposit is complete.
Anchors will no longer have to wait during the time when the account doesn't have a trust line. Instead, anchors can make the payment using ClaimableBalance operation and the stellar account (wallet user) can collect the funds at their own convenience.

**NOTE**: The Stellar Account will still need to establish trust line to the asset before claiming the outstanding balance.

**Wallets**: To support claimable balances wallets must

* Send `claimable_balance_supported=true` in their POST /transactions/deposit/interactive request.
* Periodically poll for account's available claimable balances.
* Provide a UI that allows users to claim a claimable balance. (The wallet UI should allow users to trust the asset of the claimable balance in order to claim)

**Anchors**: To support claimable balances anchors must

* Accept the `claimable_balance_supported` field in their `POST /transactions/deposit/interactive` request and submit the deposit transaction as a Claimable Balance operation to Stellar account that don't yet have a trust-line to the asset they have requested.
* Add `claimable_balance_id` to their [deposit transactions](####Fields-for-deposit-transactions)

**Anchors and Wallets**: Both anchors and wallets must also support the aforementioned ["stellar account doen't have a trust line for that asset."](#stellar-account-doesnt-trust-asset).
...
Wallets will provide `claimable_balance_supported=true` field and value in their POST parameters for `/transactions/deposit/interactive`.

If the anchor we're depositing to **supports** claimable balances.

1. Make a request to `/transactions/deposit/interactive` and provide a value for the `claimable_balance_supported=true` field.
1. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
1. The anchor will report transaction status as `pending_anchor` then make the payment as a claimable balance operation.
1. After transaction is submitted the anchor will report transaction status as `completed`

If the anchor we're depositing to **doesn't support** claimable balances.

1. Make a request to `/transactions/deposit/interactive` and provide a value for the `claimable_balance_supported=true` field.
1. When the transaction status becomes `pending_user_transfer_start` send the required payment as described in the interactive webapp callback or the `/transaction` endpoint.
1. The anchor will ignore the `claimable_balance_supported` field, discover the account does not have a trust line, and report transaction status as `pending_trust`.
1. At this point both the wallet and anchor are waiting on the trust line to be established in order to continue the deposit payment. This is described here [steps 4-6](#stellar-account-doesnt-trust-asset)

##### Wallet Feature: Claimable Balance Polling

Claimable Balance Polling allows the wallet to update the claimant that their is a balance available to claim and they may need to establish a [trust line](#stellar-account-doesnt-trust-asset) in order to do so.
...
2. The wallet can poll Horizon's [`/claimable_balances/`](https://developers.stellar.org/api/resources/claimablebalances/single/) single endpoint to pull the submitted transactions' available claimable balance(s) for the claimant(user) using the claimable_balance_id.

   * There are two recommended ways a wallet could extract the `claimable_balance_id` of the deposit transaction submitted

   1. Wallets could pull the `claimable_balance_id` from anchor's transaction deposit [transaction(s) endpoint](#transaction-history) response

   2. Wallets could decode the `results_xdr` of the deposit transaction submitted
      ...
      4. Provide UI to showcase that their deposit was successful and that their's an available claimable balance as well as enable users to [claim a claimable balance.](#wallet-feature-claim-account-claimable-balance)
...
It's recommended that Wallets have an feature that allows the account to claim the balances that were polled from Horizon.
...
1. Wallets make a request to `/transactions/deposit/interactive` and provide a value for the `claimable_balance_supported=true` field.
1. Anchors mark the transaction as [`claimable_balance_supported=true`](#deposit)
1. Anchors discover that the stellar account that their sending the asset to doesnt have a trust line.
1. The anchor should then report transaction status as `pending_anchor` and make the payment as a claimable balance operation.
1. From there the anchor will continue the [deposit flow](#basic-anchor-implementation)

##### Anchor Feature: Extend Single Historical Transaction endpoint For Claimable Balances

Wallets query the [transaction endpoint](#transaction-history) to see and display the status of deposits.
Anchors should extend their transaction endpoint response to include the `claimable_balance_id` that the anchor added to the [transaction field](####Fields-for-deposit-transactions).
...
`claimable_balance_id` | string | (optional) ID of the Claimable Balance containing the asset, included when an anchor is issuing the asset via a Claimable Balance.
