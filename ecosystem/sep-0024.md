...
[Claimable Balance](https://www.stellar.org/blog/protocol-14-improvements) is an optional feature for both wallets and anchors which enable payments to accounts that don't trust the asset being sent.
...
Anchors will no longer have to wait until the user's Stellar account has trustline to the sending asset before sending funds. Instead, anchors can make the payment using a Claimable Balance operation and the user's Stellar account can claim the funds at their own convenience.
...
**Anchors and Wallets**: Both anchors and wallets must also support the aforementioned ["Stellar account doesn't have a trustline for that asset."](#stellar-account-doesnt-trust-asset) flow.
Wallets need to be interoperable with anchors that have not adopted the claimable balance feature and vice versa.

##### Wallet Claimable Balance Flow

Wallets follow the [typical deposit flow](#Basic-Wallet-Implementation) but will provide the `claimable_balance_supported` request parameter in their POST parameters for `/transactions/deposit/interactive`.
...
Polling allows the wallet to detect claimable balances for their users. Wallets must provide a UI to users that there is a balance available to claim. See the ["Claim Account Claimable Balance"](#wallet-feature-claim-account-claimable-balance) section to learn more about how to claim a balance.
There are a few options as to how a wallet can poll claimable balance information for users.
...
This will help wallets detect that the user's deposit was successful and theres an [available claimable balance for them.](#wallet-feature-claimable-balance-polling)

##### Anchor Claimable Balance Claimants and Predicates

Predicates are one of the [claimable balance parameters](https://developers.stellar.org/docs/glossary/claimable-balance/#parameters) used to craft a Claimable Balance transaction. They're conditions to meet in order for the recipient to claim their balance.

Anchors are free to set whichever predicates they feel are necessary in order to claim the balance.
If there are no predicate preferences, `UNCONDITIONAL` allows accounts to claim balances at anytime.
