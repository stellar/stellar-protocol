...
* [`POST /transaction/attribution`](#wallet-attribution): optional (only needed if also implementing SEP-34)
...
Using [SEP-34](sep-0034.md), wallets can prove their identity to anchors for requests to process a particular transaction. This endpoint defines how the wallet should send the JWS token generated and returned from the wallet's signing server to the anchor service.

### Request

```
POST TRANSFER_SERVER/transaction/attribution
```
...
`token` | string | The JWS token returned by the wallet's [signing server](sep-0034.md)

See [Verifying Requests](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0034.md#verifying-requests) for more information about validating the tokens passed.

### Response

On successful validation of the token sent in the request, the anchor should respond with a `204` HTTP status. 

On validation error, a response body should be included along with a `400` Bad Request status code:
```json
{
  "error": "the jws was not signed by the public key listed in the 'iss' stellar.toml'"
}
```
