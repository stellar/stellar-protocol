...
* [`GET /deposit-exchange-with-market-rate`](#deposit-exchange-with-market-rate): optional
* [`GET /deposit-exchange-with-quote`](#deposit-exchange-with-quote): optional
* [`GET /withdraw`](#withdraw): required
* [`GET /withdraw-exchange-with-market-rate`](#withdraw-exchange-with-market-rate): optional
* [`GET /withdraw-exchange-with-quote`](#withdraw-exchange-with-quote): optional
* [`GET /info`](#info): required
* [`GET /fee`](#fee): optional (only needed for complex fee structures)
* [`GET /fee-deposit-quote`](#fee-for-quoted-deposits): optional (only needed for complex fee structures)
* [`GET /fee-withdraw-quote`](#fee-for-quoted-withdrawals): optional (only needed for complex fee structures)
...
To enable the conversion of non-equivalent assets, the Anchor needs to implement the [SEP-38 Anchor RFQ API](sep-0038.md), which enables the Anchor to provide quotes to the Wallet, as well as the endpoints [`GET /deposit-exchange-with-market-rate`](#deposit-exchange-with-market-rate) and [`GET /withdraw-exchange-with-market-rate`](#withdraw-exchange-with-market-rate) for conversions using the market rate (AKA [indicative quotes](https://www.investopedia.com/terms/i/indicativequote.asp)), and [`GET /deposit-exchange-with-quote`](#deposit-exchange-with-quote) and [`GET /withdraw-exchange-with-quote`](#withdraw-exchange-with-quote) for [firm quotes](https://www.investopedia.com/terms/f/firmquote.asp).

### Asset Conversions Using the Market Rate

To convert at the market rate, should previously get an indicative quote but there won't be a firm guarantee of the asset price they are going to pay. Here is how the conversion should happen in deposits and withdrawals for an asset pair like `BRL <> USDC`:

**Deposit:**

Considering the user will be making a bank deposit of fiat `BRL` in exchange for `USDC` in the Stellar network:

1. The Wallet checks the assets' market conversion rate in [`SEP-38 GET /price`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0038.md#get-price) and shows the estimated price to the user.
1. The Wallet initiates a [`GET /deposit-exchange-with-market-rate`](#deposit-exchange-with-market-rate) using the parameters `asset_code` and `source_asset`.
...
1. The Wallet initiates a [`GET /withdraw-exchange-with-market-rate`](#withdraw-exchange-with-market-rate) using the parameters `asset_code` and `destination_asset`.
...
With firm quotes, users can have a guarantee of the asset price they are going to pay **if they submit the payment before the quote expiration**. Here is how the conversion should happen in deposits and withdrawals for an asset pair like `USDC <> BRL`:
...
1. The Wallet initiates a [`GET /deposit-exchange-with-quote`](#deposit-exchange-with-quote) using the parameter `quote_id` returned from the previous step.
...
1. The Wallet gets a firm conversion rate in [`SEP-38 POST /quote`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0038.md#post-quote) and shows the price and expiration date to the user.
1. The Wallet initiates a [`GET /withdraw-exchange-with-quote`](#withdraw-exchange-with-quote) using the parameter `quote_id` returned from the previous step.
...
If the anchor supports [SEP-38] quotes, it can also provide a bridge between non-equivalent tokens by receiving, for instance ARS via bank transfer and in return sending the equivalent value (minus fees) as USDC on the Stellar network to the user's Stellar account. That kind of deposit is covered in [`GET /deposit-exchange-with-market-rate`](#deposit-exchange-with-market-rate) and [`GET /deposit-exchange-with-quote`](#deposit-exchange-with-quote).
...
## Deposit Exchange with Market Rate
...
The `/deposit-exchange-with-market-rate` endpoint allows a wallet to get deposit information from an anchor when the user intends to make a conversion between non-equivalent tokens at the market rate. With this endpoint, a user has all the information needed to initiate a deposit and it also lets the anchor specify additional information (if desired) that the user must submit via [SEP-12](sep-0012.md).

### Request

```curl
GET TRANSFER_SERVER/deposit-exchange-with-market-rate
```
...
Name | Type | Description
-----|------|------------
`asset_code` | string | The code of the **on-chain** asset the user wants to get from the Anchor after doing an off-chain deposit. The value passed must match one of the codes listed in the [/info](#info) response's `deposit-exchange-with-market-rate` object.
...
```curl
GET https://api.example.com/deposit-exchange-with-market-rate?asset_code=USDC&source_asset=iso4217:BRL&account=GACW7NONV43MZIFHCOKCQJAKSJSISSICFVUJ2C6EZIW5773OU3HD64VI
```
...
## Deposit Exchange with Quote

If the anchor supports [SEP-38] quotes, it can provide a deposit that makes a bridge between non-equivalent tokens by receiving, for instance BRL via bank transfer and in return sending the equivalent value (minus fees) as USDC to the user's Stellar account.

The `/deposit-exchange-with-quote` endpoint allows a wallet to get deposit information from an anchor when the user intends to make a conversion between non-equivalent tokens using a [firm quote](https://www.investopedia.com/terms/f/firmquote.asp). With this endpoint, a user has all the information needed to initiate a deposit and it also lets the anchor specify additional information (if desired) that the user must submit via [SEP-12](sep-0012.md).

### Request

```curl
GET TRANSFER_SERVER/deposit-exchange-with-quote
```
...
```curl
GET https://api.example.com/deposit-exchange-with-quote?quote_id=asdf1234&account=GACW7NONV43MZIFHCOKCQJAKSJSISSICFVUJ2C6EZIW5773OU3HD64VI
```
...
If the anchor supports [SEP-38] quotes, it can also provide a bridge between non-equivalent tokens by receiving, for instance USDC from the Stellar network and in return sending the equivalent value (minus fees) as NGN to the user's bank account. That kind of withdrawal is covered in [`GET /withdraw-exchange-with-market-rate`](#withdraw-exchange-with-market-rate) and [`GET /withdraw-exchange-with-quote`](#withdraw-exchange-with-quote).
...
## Withdraw Exchange with Market Rate
...
The `/withdraw-exchange-with-market-rate` endpoint allows a wallet to get withdraw information from an anchor when the user intends to make a conversion between non-equivalent tokens using the market rate. With this endpoint, a user has all the information needed to initiate a withdraw and it also lets the anchor specify additional information (if desired) that the user must submit via [SEP-12](sep-0012.md).

### Request

```
GET TRANSFER_SERVER/withdraw-exchange-with-market-rate
```
...
Name | Type | Description
-----|------|------------
`asset_code` | string | Code of the **on-chain** asset the user wants to withdraw. The value passed must match one of the codes listed in the [/info](#info) response's `withdraw-exchange-with-market-rate` object.
...
```curl
GET https://api.example.com/withdraw-exchange-with-market-rate?asset_code=USDC&destination_asset=iso4217:NGN&dest=0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe
```
...
The `/withdraw-exchange-with-quote` endpoint allows a wallet to get withdraw information from an anchor when the user intends to make a conversion between non-equivalent tokens using a [firm quote](https://www.investopedia.com/terms/f/firmquote.asp). With this endpoint, a user has all the information needed to initiate a withdraw and it also lets the anchor specify additional information (if desired) that the user must submit via [SEP-12](sep-0012.md).

### Request

```
GET TRANSFER_SERVER/withdraw-exchange-with-quote
```
...
```curl
GET https://api.example.com/withdraw-exchange-with-quote?quote_id=asdf1234&dest=0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe
```
...
```js
{
  ...
  "deposit-exchange-with-market-rate": {
    "USD": {
      "authentication_required": true,
      "fee_fixed": 5,    // deposit fees are charged in units of the off-chain asset
      "fee_percent": 1,  // deposit fees are charged in units of the off-chain asset
      "fields": {
        "email_address" : {
          "description": "your email address for transaction status updates",
          "optional": true
  ...
  "withdraw-exchange-with-market-rate": {
    "USD": {
      "authentication_required": true,
      "fee_fixed": 5,    // withdrawal fees are charged in units of the asset on Stellar
      "fee_percent": 0,  // withdrawal fees are charged in units of the asset on Stellar
      "min_amount": 0.1,
      "max_amount": 1000,
      "types": {
        "bank_account": {
...
_Note: the attributes above mentioned are valid for all kinds of deposits, including `deposit`, `deposit-exchange-with-market-rate` and `deposit-exchange-with-quote`._
...
_Note: the attributes above mentioned are valid for all kinds of withdrawals, including `withdraw`, `withdraw-exchange-with-market-rate` and `withdraw-exchange-with-quote`._
...
The fee endpoint allows an anchor to report the fee that would be charged for a given deposit or withdraw operation between equivalent assets. For non-equivalent assets please refer to the [Fee for Quoted Deposits](#fee-for-quoted-deposits) and [Fee for Quoted Withdrawals](#fee-for-quoted-withdrawals) sections.
...
## Fee for Quoted Deposits

This endpoint allows an anchor to report the fee that would be charged for a quoted deposit operation between non-equivalent assets. This endpoint is important to allow an anchor to accurately report fees to a user even when the fee schedule is complex. If a fee can be fully expressed with the `fee_fixed` and `fee_percent` fields in the `/info` response, then an anchor should not implement this endpoint.

```curl
GET TRANSFER_SERVER/fee-deposit-quote
```
...
Name | Type | Description
-----|------|------------
`operation` | string | Kind of operation (`deposit-exchange-with-market-rate` or `deposit-exchange-with-quote`).
`type` | string | (optional) Type of deposit (`SEPA`, `bank_account`, `cash`, etc...).
`asset_code` | string | The code of the **on-chain** asset the user wants to get from the Anchor after doing an **off-chain** deposit. The value passed must match one of the codes listed in the [/info](#info) response's `deposit-exchange-with-market-rate` or `deposit-exchange-with-quote` objects.
`source_asset` | string | The **off-chain** asset the Anchor will receive from the user. The value must match one of the `asset` values included in a [`SEP-38 GET /prices?buy_asset=stellar:<asset_code>:<asset_issuer>`](sep-0038.md#get-prices) response using [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format).
`amount` | string | The amount of the **source_asset** the user would like to deposit to the anchor's **off-chain** account.

Example request:

```curl
GET https://api.example.com/fee-deposit-quote?operation=deposit-exchange-with-quote&source_asset=iso4217:NGN&amount=375.5&asset_code=ETH
```
...
Name | Type | Description
-----|------|------------
`fee` | float | The total fee, in units of the **off-chain** asset, that would be charged to deposit the specified `amount` of `source_asset`.
...
## Fee for Quoted Withdrawals

This endpoint allows an anchor to report the fee that would be charged for a quoted withdraw operation between non-equivalent assets. This endpoint is important to allow an anchor to accurately report fees to a user even when the fee schedule is complex. If a fee can be fully expressed with the `fee_fixed` and `fee_percent` fields in the `/info` response, then an anchor should not implement this endpoint.

```curl
GET TRANSFER_SERVER/fee-withdraw-quote
```
...
Name | Type | Description
-----|------|------------
`operation` | string | Kind of operation (`withdraw-exchange-with-market-rate` or `withdraw-exchange-with-quote`).
`type` | string | (optional) Type of withdrawal (`SEPA`, `bank_account`, `cash`, etc...).
`asset_code` | string | Code of the **on-chain** asset the user wants to withdraw. The value passed must match one of the codes listed in the [/info](#info) response's `withdraw-exchange-with-market-rate` or `withdraw-exchange-with-quote` objects.
`destination_asset` | string | The **off-chain** asset the Anchor will deliver to the user's account. The value must match one of the `asset` values included in a [`SEP-38 GET /prices?sell_asset=stellar:<asset_code>:<asset_issuer>`](sep-0038.md#get-prices) response using [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format).
`amount` | float | Amount of the **on-chain** asset the user would like to send to the anchor's Stellar account.

Example request:

```curl
GET https://api.example.com/fee-withdraw-quote?operation=withdraw-exchange-with-market-rate&asset_code=ETH&amount=0.5&destination_asset=iso4217:NGN
```
...
Name | Type | Description
-----|------|------------
`fee` | float | The total fee, in units of the **on-chain** asset, that would be charged to withdraw the specified `amount` of `asset_code`.
...
`kind` | string | One of the following: `["deposit", "deposit-exchange-with-market-rate", "deposit-exchange-with-quote", "withdrawal", "withdrawal-exchange-with-market-rate", "withdrawal-exchange-with-quote"]`.
...
```js
{
  "transactions": [
    ...
    {
      "id": "52fys79f63dh3v2",
      "kind": "deposit-exchange-with-market-rate", // or deposit-exchange-with-quote
    ...
    {
      "id": "52fys79f63dh3v2",
      "kind": "withdrawal-exchange-with-market-rate", // or withdrawal-exchange-with-quote
      "status": "pending_anchor",
      "status_eta": 3600,
      "stellar_transaction_id": "17a670bc424ff5ce3b386dbfaae9990b66a2a37b4fbe51547e8794962a3f9e6a",
      "amount_in": "100",
