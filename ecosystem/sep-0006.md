...
If the issuer needs more information about the customer and all the information can be received non-interactively via the `/customer` endpoint, this is the correct response. Once the user / wallet transmits information to the `/customer` endpoint, they can retry the `/federation?type=forward` endpoint to see if the issuer is ready to execute the withdrawal.
...
If the wallet wants to be notified that the user has completed the required actions via the URL, it can add an extra `callback` parameter to the value of `url` before opening the browser window. If the `callback` value is a URL, the issuer should visit it once the user has successfully completed the process. If `callback=postMessage` is passed, the issuer should post `success` to `window.opener` via the Javascript `Window.postMessage` method. Alternately, the wallet can always poll the `/federation&type=forward` endpoint until a withdrawal address is returned.
...
```
GET https://api.example.com/jwt?account=GCIBUCGPOHWMMMFPFTDWBSVHQRT4DIBJ7AD6BZJYDITBK2LCVBYW7HUQ&nonce=Pq6WVhFd55yojWmLbxVY5clUEifrK9AIQyhmmchEt0wR&signature=ASDF
```
