...
Client applications can use a single Stellar account to hold multiple users' funds. To distinguish users that use the same Stellar account, the decoded SEP-10 JWT's `sub` value may also contain a memo value. The anchor should use this attribute in addition to the Stellar account in their data model to identify users. See the [SEP-10 Memos](sep-0010.md#memos) section for more information.
