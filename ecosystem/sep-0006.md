...
* [`GET /token`](#token): optional, but necessary for authentication (step 2)
* [`POST /customer`](#customer): optional, but necessary if anchor desires any information about the customer for KYC or other purposes. Requires authentication (`/nonce` and `/token`)
...
```
GET https://apay.io/api/deposit?asset_code=ETH&account=GACW7NONV43MZIFHCOKCQJAKSJSISSICFVUJ2C6EZIW5773OU3HD64VI
```
...
If the wallet wants to be notified that the user has completed the required actions via the URL, it can add an extra `callback` parameter to the value of `url` before opening the browser window. If the `callback` value is a URL, the issuer should visit it once the user has successfully completed the process. If `callback=postMessage` is passed, the issuer should post `success` to `window.opener` via the Javascript `Window.postMessage` method. Alternately, the wallet can always poll the `/deposit` endpoint until a deposit location is returned.
...
```json
{
  ...
  "url" : "https://www.example.com/kycflow?account=GACW7NONV43MZIFHCOKCQJAKSJSISSICFVUJ2C6EZIW5773OU3HD64VI"
}
```
...
```json
{
  "type": "status",
  "status": "denied",
  "more_info_url": "https://www.example.com/kycstatus?account=GACW7NONV43MZIFHCOKCQJAKSJSISSICFVUJ2C6EZIW5773OU3HD64VI"
}
```

#### 5. Error

Every HTTP status code other than `200 OK` and `403 Forbidden` will be considered an error. The body should contain error details.
For example:
...
Name | Type | Description
-----|------|------------
`account` | string | The stellar account that the wallet wishes to authenticate to the issuer

Example:

```
GET https://www.example.com/nonce?account=GCIBUCGPOHWMMMFPFTDWBSVHQRT4DIBJ7AD6BZJYDITBK2LCVBYW7HUQ
```
...
Name | Type | Description
-----|------|------------
`nonce` | string | A cryptographic-quality random string base64 URL encoded with at least 128 bits of entropy
...
`signature` | string | A base64 URL encoded signature of the nonce string generated with the secret key to the `account`

Example:

```
GET https://www.example.com/token?account=GCIBUCGPOHWMMMFPFTDWBSVHQRT4DIBJ7AD6BZJYDITBK2LCVBYW7HUQ&nonce=Pq6WVhFd55yojWmLbxVY5clUEifrK9AIQyhmmchEt0wR&signature=ASDF
```
