...
Client applications can use a single Stellar account to hold multiple users' funds. To distinguish users that use the same Stellar account, the decoded SEP-10 JWT's `sub` value may also contain a memo value. The anchor should use this attributes in addition to the Stellar account in their data model to identify users. See the [SEP-10 Memos](sep-0010.md#memos) section for more information.
...
`memo` | string | (optional) This field should only be used if SEP-10 authentication is not. Must be a It was originally intended to distinguish users of the same Stellar account. However, the anchor should use the memo portion of the `sub` value included in the decoded SEP-10 JWT instead if present. Anchors should still support this parameter to maintain support for outdated clients. See the [Shared Account Authentication](#shared-accounts) section for more information.
