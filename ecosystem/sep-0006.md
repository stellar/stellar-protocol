...
```json
{
  ...
  "withdraw": {
    "USD": {
      ...
      "types": {
        "bank_account": {
          "extra_fields": ["bank_branch", "phone_number"],
          "instructions": "Enter your email address as 'dest' and specify your bank branch's address in 'bank_branch'"
        },
        "cash": {
          "instructions": "Enter your email address as 'dest'. Your cashout PIN will be emailed to you."
        }
       }
    },
    "ETH": {
...
For each asset it contains:

* The fee structure
* `extra_fields`: Any extra non-standard or normally optional fields that must be sent for the `/deposit` or `/withdraw` request to succeed. Bank name or bank address, for example, may be necessary for a wire transfer to withdraw USD. The wallet will prompt the user to fill out these fields as part of the deposit/withdrawal flow.

For withdrawal assets, an object of types of withdrawal supported. The anchor may specify the following fields for each type:

* `instructions`: Instructions to user on how to fill in the `dest`, `dest_extra` and any extra fields for each type of withdrawal
* `extra_fields`: Extra fields needed for this type of withdrawal
...
An anchor can implement this endpoint to allow user authentication. The endpoint must return a random base64-encoded string with at least 128 bits of entropy. A wallet makes a request to this endpoint to begin the authentication process. A random, anchor-picked value is essential for an authentication process that is resistant to replay attacks. The anchor must store the `<nonce, account>` pair for 5-10 minutes, after which it should be deleted. An anchor is at liberty to rate limit requests as needed.

### Request

```
GET TRANSFER_SERVER/nonce
```
...
An anchor can implement this endpoint to allow user authentication. The endpoint accepts a nonce, an account, and a signature of that nonce with the secret key for the account. If the signature is valid, it responds with a [JSON Web Token](https://jwt.io/) authenticating the user. An anchor can pick its own expiration period for the token, however 24 hours is recommended.

### Request

```
GET TRANSFER_SERVER/token
```
...
-----|------|------------
`account` | `G...` string | The stellar account that the wallet wishes to authenticate to the anchor
`nonce` | string | A valid nonce that was recently returned by the anchor for this account
`signature` | string | A base64 signature of the nonce string (do not base64 decode the nonce prior to signing) generated with the secret key to the `account`

Note that since both `nonce` and `signature` are base64 encoded, they'll also need to be URL-encoded before being passed in the `GET` request.
...
If the signature and nonce are both valid, the anchor generates a JWT and puts it in the response body. The JWT must encode the user's account ID in the `sub` field as well as the time of issuance. No other fields are needed.
