...
Anchors may support [SEP-10](sep-0010.md) web authentication to enable
...
1. clients must submit the JWT previously obtained from the anchor via the
   [SEP-10](sep-0010.md) authentication flow to all API endpoints.
...
- **Use the `/info` endpoint**
  ...
  - While `/info` allows an anchor to communicate fields that are needed for
    ...
    correctly.
- **Authentication**
  - If needed, perform [authentication](#authentication) via SEP-10 before
