...
`account` | `G...` string | (**deprecated**) This field should match the Stellar account authenticated via SEP-10. Anchors should use the public key assigned to the `sub` field of the decoded JWT as the account to send funds to. See the [Shared Account Authentication](#shared-accounts) section for more information.
...
`account` | `G...` string | (**deprecated**, optional) This field should match the Stellar account authenticated via SEP-10 if present. Anchors should use the public key assigned to the `sub` field of the decoded JWT as the account to send funds to. See the [Shared Account Authentication](#shared-accounts) section for more information.
`memo` | string | (**deprecated**, optional) This field was originally intended to differentiate users of the same Stellar account. However, the anchor should use the `memo` and `memo_type` values included in the decoded SEP-10 JWT instead. Anchors should still support this parameter to maintain support for outdated clients. See the [Shared Account Authentication](#shared-accounts) section for more information.
`memo_type` | string | (**deprecated**, optional) Type of `memo`. One of `text`, `id` or `hash`. Use the `memo_type` value included in the decoded SEP-10 JWT instead.
