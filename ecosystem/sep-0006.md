...
Get instructions for depositing an asset with the anchor. `DEPOSIT_SERVER` should be specified in the `stellar.toml` of the anchor.

```
Endpoint: DEPOSIT_SERVER/deposit
Method: GET
```
...
If the anchor requires KYC data prior to deposit, it should specify the required KYC fields in its [`stellar.toml`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0001.md) and implement the [KYC Upload](#kyc-upload) endpoint.
...
```
Endpoint: FEDERATION_SERVER/federation
Method: GET
```
...
If the anchor requires KYC data prior to withdrawal, it should specify the required KYC fields in its [`stellar.toml`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0001.md) and implement the [KYC Upload](#kyc-upload) endpoint.
...
```
Endpoint: DEPOSIT_SERVER/transactions
Method: GET
```
...
The KYC upload endpoint should be implemented by anchors that require KYC info before accepting deposits or allowing withdrawals. The endpoint accommodates KYC data that is large or binary formatted (image of driver's license, photo of bill for proof of address, etc...). An anchor receiving KYC data should store it securely for the duration of time specified by the client.

```
Endpoint: DEPOSIT_SERVER/kyc
Method: POST
Content-Type: multipart/form-data
```

The fields below should be placed in the `POST` body using the `multipart/form-data` encoding.

Name | Type | Description
-----|------|------------
`account` | string | The account to upload KYC data for. Can be a Stellar account ID, external crypto account id, or external bank account number.
...
`access_desired` | `deposit`, `withdraw`, `both` | The kind of access the customer is requesting. An anchor can skip the withdraw approval process if the customer only wants deposit approval, for example.
... | any | All of the required KYC fields specified in [`stellar.toml`](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0001.md), filled in with information about the account holder

On success the endpoint should return `200 OK` HTTP status code and a JSON object with the following fields:

Name | Type | Description
-----|------|------------
`deposit_status` | `ok`, `denied`, `pending`, `missing` | Status of customer approval to deposit funds. `missing` if more info is needed.
`withdraw_status` | `ok`, `denied`, `pending`, `missing` | Status of customer approval to withdraw funds. `missing` if more info is needed.
`pending` | int | Estimated number of seconds until approval decisions will be made. When approval comes through, the [Deposit](#deposit) and [Withdraw](#withdraw) endpoints will stop returning errors and will return deposit/withdraw information.
`details` | string | Textual description of decisions or the approval process for the customer to read
...
```json
{
  ...
  "pending": 86400,
  "details": "Your drivers license must be verified prior to withdraw approval. It usually takes a day",
}
```
...
```json
{
   "error": "Image cannot be decoded. Must be jpg or png."
}
```
