...
| `customer_id`                 | string | (optional) id of an off-chain account (managed internally by the anchor) associated with this user's Stellar account (identified by JWT). If the anchor supports [SEP-12], the `customer_id` field should match [SEP-12] customer's id. `customer_id` should be passed only when off-chain id is know to the client, but relationship between this id and user's Stellar account is not known to the Anchor.                                                                                                                                                                                                                                                                                               |
