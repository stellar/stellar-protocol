...
- `expires_at`: (if specified) a timestamp representing the expiration date for the anchor to receive that payment and proceed with the deposit/withdraw at the agreed quote. This value will be present if and only if the deposit/withdraw was submitted using the `quote_id` parameter. If already expired, the wallet should not allow the user to proceed with the deposit/withraw action.
