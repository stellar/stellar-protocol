...
`amount_fee` | string | (optional) Amount of fee charged by anchor. If a `quote_id` was used and the quote provides `fee_details`, the SEP-6 `amount_fee` should be `sum(quote.fee_details.amount)`.
`amount_fee_asset` | string | (optional) The asset in which fees are calculated in. Must be present if the deposit/withdraw was made using quotes. The value must be in [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format). If a `quote_id` was used and the quote provides `fee_details`, the SEP-6 `amount_fee_asset` should be `any(quote.fee_details.asset)`, since SEP-38 does not allow more than one asset in the fee details.
...
* `v3.11.1`: App information about how a SEP-6 transaction will populate the `amount_fee` and `amount_fee_asset` fields when a `quote_id` is used. ([#1204](https://github.com/stellar/stellar-protocol/pull/1204))
