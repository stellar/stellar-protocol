...
A withdraw is when a user redeems an asset currently on the Stellar network for it's equivalent off-chain asset via the Anchor. For instance, a user redeeming their NGNT in exchange for fiat NGN.
...
- `expires_at`: (if specified) a timestamp representing the expiration date for the anchor to receive that payment at the agreed quote. This value will only be present if the withdraw was submitted using the `quote_id` parameter. If already expired, the wallet should not allow the user to proceed with the withraw action.
