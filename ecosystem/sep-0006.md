...
* **Use the `/info` endpoint**
    ...
    * The recommended fee formula for withdrawals and deposits is: `O = (I - F)(1 - P)` where `O` = Output (amount the recipient will receive), `I` = Input (amount that will be sent to anchor), `F` = Fixed Fee and `P` = Percentage Fee.
