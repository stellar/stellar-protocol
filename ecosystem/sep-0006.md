...
```
SEP: 0006
Title: Non-Interactive Anchor/Wallet Interface
Author: SDF
Status: Active
Created: 2017-10-30
Updated: 2020-03-28
...
This SEP defines the standard backend implementation for anchors and wallets to interact on behalf of users. It provides normalized interface specifications that allow wallets and other services to interact with anchors programmatically. 

While this document specifies a **non-interactive interface** between wallets and anchors, which is based on REST based backend APIs without any browser dependencies, [SEP-24](sep-0024.md) specifies an **interactive interface** using frontend UIs that collect complex KYC information potentially required by anchors. Both SEPs achieve the same thing in different ways and can be implemented alongside each other as as standalone interface.  
...
* **Use the `/info` endpoint**
    ...
    * The recommended fee formula for withdrawals and deposits is: `O = (I - F)(1 - P)` where `O` = Output (amount the recipient will receive), `I` = Input (amount that will be sent to anchor), `F` = Fixed Fee and `P` = Percentage Fee. Double check with the anchor on this their actual formula.
...
* **For `/deposit`**
    * If the issuer's `/deposit` endpoint immediately returns success:
        * display the deposit info that came back from the endpoint to the user, including fee. At ths point the user can execute the deposit using the instructions received from the anchor.
    * Handle [special cases](#special-cases).
* **For `/withdraw`**
    * If the issuer's `/withdraw` endpoint immediately returns success:
        * Provide an interface to the user that allows them to review the withdrawal details including the computed fee. Upon successful review and user confirmation the wallet initiates the withdrawal by sending the Stellar payment to the address provided by the anchor.
* **Transaction history**
...
* The recommended fee formula for withdrawals and deposits is: `O = (I - F)(1 - P)` where `O` = Output (amount the recipient will receive), `I` = Input (amount that will be sent to anchor), `F` = Fixed Fee and `P` = Percentage Fee. Use this formula unless you provide a more complex fee structure via the `/fee` endpoint.
...
The `fields` object allows an anchor to describe fields that are passed into `/deposit` and `/withdraw`. It can explain standard fields like `dest` and `dest_extra` for withdrawal, and it can also specify extra fields that should be passed into `/deposit` or `/withdraw` such as an email address or bank name. If a field is part of the KYC/AML flow handled by SEP-12 there's no need to list it in `/info`. Only fields that are passed to `/deposit` or `/withdraw` need appear here.
