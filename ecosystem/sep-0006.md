...
`url` | string | URL hosted by the anchor. The wallet should show this URL to the user either as a popup or an iframe. IMPORTANT: If the client application obtained a JWT via [SEP0010](sep-0010.md), the client app should add it as an `Authorization: Bearer <jwt>` header when requesting from this `url`.
