...
Client applications can use a single Stellar account to hold multiple users' funds. To differentiate users using the same Stellar account, the decoded SEP-10 JWT may contain `memo` and `memo_type` attributes. The anchor should use these attributes in addition to the Stellar account in their data model to represent unique users. See the [SEP-10 Memos](sep-0010.md#memos) section for more information.
...
`memo` | string | (optional) Value of memo to attach to transaction, for `hash` this should be base64-encoded. This field, as well as `memo_type`, can be different than the values included in the decoded SEP-10 JWT.
...
If the decoded JWT also contains the `memo` and `memo_type` parameters, the anchor must only return transactions for the user identified by the `memo`, _not all transactions for the Stellar account_.
