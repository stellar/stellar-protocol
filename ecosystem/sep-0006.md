...
`dest` | string | The account to which the user wants to withdraw their funds. This can be a crypto account, a bank account number, or IBAN.
`dest_extra` | string | (optional) Extra information to specify withdrawal location. For crypto it may be a memo in addition to the `dest` address. It can also be a routing number for a bank or a BIC.
...
```json
{
  ...
  "withdraw": {
    "USD": {
      ...
      "fee_fixed": 0,
      "fee_percent": 0,
      "extra_fields": ["bank_name", "bank_address"]
    },
    "ETH": {
      "enabled": true,
      "fee_fixed": 0.002,
...
* For which assets the anchor supports SEP6 deposit and/or withdrawal
* The fee structure for those assets
* Any extra non-standard fields that must be sent with the `/deposit` or `/withdraw` request. Bank name or bank address, for example, may be necessary for a wire transfer to withdraw USD. The wallet will prompt the user to fill out these fields as part of the deposit/withdrawal flow.
