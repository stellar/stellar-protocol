...
In either case, the JSON message should the same as the [Customer Information Status](#4-customer-information-status) response format, with one change. Since it's possible that the anchor is `POST`ing or `postMessaging` a success result, the `status` field may also be set to `success`.

As an alternative to using the `callback` parameter (or in addition to get more information), the wallet can poll the transaction endpoint [`/transaction`](#single-historical-transaction) with the request `id` to check the status of the request.
