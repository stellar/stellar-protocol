...
-----|------|------------
`asset_code` | string | (optional) The code of the **on-chain** asset the user wants to get from the Anchor after doing an off-chain deposit. The value passed must match one of the codes listed in the [/info](#info) response's deposit object. This field is mandatory if `quote_id` is not set.
...
`quote_id` | string | (optional) The `id` returned from a `SEP-38 POST /quote` response. If this attribute is specified, none of the attributes `asset_code`, `amount` or `source_asset` should be included in the request. The user _must_ deliver the deposit funds to the Anchor before the quote's expiration defined in the quote's `expires_at` attribute.
...
Name | Type | Description
-----|------|------------
`asset_code` | string | (optional) Code of the **on-chain** asset the user wants to withdraw. The value passed must match one of the codes listed in the [/info](#info) response's withdraw object. This field is mandatory if `quote_id` is not set.
