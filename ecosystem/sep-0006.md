...
* [`GET /fee_deposit_quote`](#fee-for-quoted-deposits): optional (only needed for complex fee structures)
* [`GET /fee_withdraw_quote`](#fee-for-quoted-withdrawals): optional (only needed for complex fee structures)
...
```json
{
  ...
  "withdraw_indicative_quote": {
    "USD": {
      "authentication_required": true,
      "fee_fixed": 5,    // withdraw fees are charged in units of the asset on Stellar
      "fee_percent": 0,  // withdraw fees are charged in units of the asset on Stellar
      "min_amount": 0.1,
      "max_amount": 1000,
      "types": {
        "bank_account": {
...
Name | Type | Description
-----|------|------------
`operation` | string | Kind of operation (`deposit`, `deposit_indicative_quote`, `deposit_firm_quote`, `withdraw`, `withdraw_indicative_quote` or `withdraw_firm_quote`).
...
```curl
GET TRANSFER_SERVER/fee_deposit_quote
```
...
Name | Type | Description
-----|------|------------
`operation` | string | Kind of operation (`deposit_indicative_quote` or `deposit_firm_quote`).
`type` | string | (optional) Type of deposit (`SEPA`, `bank_account`, `cash`, etc...).
`asset_code` | string | The code of the **on-chain** asset the user wants to get from the Anchor after doing an **off-chain** deposit. The value passed must match one of the codes listed in the [/info](#info) response's `deposit_indicative_quote` or `deposit_firm_quote` objects.
`source_asset` | string | The **off-chain** asset the Anchor will receive from the user. The value must match one of the `asset` values included in a [`SEP-38 GET /prices?buy_asset=stellar:<asset_code>:<asset_issuer>`](sep-0038.md#get-prices) response using [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format).
...
```curl
GET https://api.example.com/fee_deposit_quote?operation=deposit_firm_quote&source_asset=iso4217:NGN&amount=375.5&asset_code=ETH
```
...
```curl
GET TRANSFER_SERVER/fee_withdraw_quote
```
...
Name | Type | Description
-----|------|------------
`operation` | string | Kind of operation (`withdraw_indicative_quote` or `withdraw_firm_quote`).
`type` | string | (optional) Type of withdrawal (`SEPA`, `bank_account`, `cash`, etc...).
`asset_code` | string | Code of the **on-chain** asset the user wants to withdraw. The value passed must match one of the codes listed in the [/info](#info) response's `withdraw_indicative_quote` or `withdraw_firm_quote` objects.
`destination_asset` | string | The **off-chain** asset the Anchor will deliver to the user's account. The value must match one of the `asset` values included in a [`SEP-38 GET /prices?sell_asset=stellar:<asset_code>:<asset_issuer>`](sep-0038.md#get-prices) response using [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format).
...
```curl
GET https://api.example.com/fee_withdraw_quote?operation=withdraw_indicative_quote&asset_code=ETH&amount=0.5&destination_asset=iso4217:NGN
```
