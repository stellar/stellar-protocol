...
Bitcoin response example (using deprecated `how` field):
...
Ripple response example (using deprecated `how` field):
...
Mexican peso (MXN) response example (using deprecated `how` field):
...
`required_customer_info_message` | string | (optional) A human-readable message indicating any errors that require updated information from the user.
