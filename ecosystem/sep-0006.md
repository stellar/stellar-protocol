...
- `status`: `pending_user_transfer_complete` means the payment had been successfully received by the anchor and the funds is ready for customer to pick up. The instruction to pick up the funds should be provided to the customer, most likely through the `more_info_url` page provided by the anchor in the response of `/transaction`.
...
* `pending_user_transfer_complete` means the payment had been successfully received by the anchor and the funds is ready for customer to pick up. 
