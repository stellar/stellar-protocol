...
Asset names, form fields, fee structures ... shown in the examples below may need to be presented in different languages other than in English.

For localized stellar.toml configurations,  add the query parameter `?lang=languageCode` to your stellar.toml endpoint. **languageCode** is the ISO 639-1 language code for the desired language.

For example, https://homedomain.com/.well-known/stellar.toml?lang=es will load a stellar.toml configuration with spanish translations.
...
### [[sep26.deposits.options]] (Stellar.toml)
...
* `type`: can be number, text, email, or choice. By default, the form field type is text.
