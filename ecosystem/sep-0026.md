...
```
SEP: 0026
Title: Non-interactive Anchor/Client interoperability
Author: SDF, @msfeldstein
Status: Draft
Created: 2019-11-11
Updated: 2019-11-11
Version 1.0.0
...
* Handle anchor KYC needs, including transmitting KYC information about the user to the anchor via POST parameters to the `deposit` or `withdraw` endpoints
...
* [`POST /transactions/deposit/non-interactive`](#deposit)
* [`POST /transactions/withdraw/non-interactive`](#withdraw)
* [`GET /info`](#info)
...
As stated, Anchors will support [SEP-10](sep-0010.md) authentication to enable deposits, withdrawals, or transaction history lookups to be authenticated via a stellar key.

The JWT should be included in all requests as a request header:
```
...
* **Authentication**
   * Perform [authentication](#authentication) via SEP-10 in order to access any endpoints involving user data (anything besides `/info` or `/fee`).
* **Make a request to `/transactions/deposit/non-interactive` or `/transactions/withdraw/non-interactive`.**
    * Optionally, pass in `amount` and any [SEP-9](sep-0009.md) fields that the wallet has on-hand, such as `email_address` and `account`.
* **For `deposit`**
...
* **For `withdraw`**
    * Have the user enter the crypto account or bank account where they'd like their withdrawal to end up, and provide that to the anchor via `dest` and `dest_extra`.
...
* Pick your approach to [fees](#fee). We recommend using `/info` to express fees as it provides a better user experience (the user can see the fee structure in the wallet early in the process).
* **For both deposit and withdrawal**:
    * Include the `id` field in your response so the wallet can check up on the status of the transaction at any time.
    * Make sure to pass along the `amount` to the `/deposit` or `/withdraw` endpoint if the wallet collects it.
* **Providing transaction status**
    * Provide the `/transaction` endpoint. The wallet relies on it to determine status of long-lived transactions.
...
### 43 Customer Information Status
...
The JSON object contains an entry for each asset that the anchor supports for SEP26 deposit and/or withdrawal.

#### For each deposit asset, response contains:

* `enabled`: `true` if SEP-26 deposit for this asset is supported
...
The `fields` object allows an anchor to describe fields that are passed into `/transactions/deposit/non-interactive` and `/transactions/withdraw/non-interactive`. It can explain standard fields like `dest` and `dest_extra` for withdrawal, and it can also specify extra fields that should be passed into `/transactions/deposit/non-interactive` or `/transactions/withdraw/non-interactive` such as an email address or bank name.
...
* `incomplete` -- there is not yet enough information for this transaction to be initiated.
