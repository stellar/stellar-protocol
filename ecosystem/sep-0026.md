...
Certain anchored assets may support the ability to fund unfunded accounts. In
that case, the anchor should define the `fund_wallet` field under the `sep26`
field.
...
Deposit metadata for each anchored asset is specified inside the `deposits`
field
...
See the [BTC SEP-27 example response](#btc-example) above of an asset that uses
the choice type on a withdrawal field.
...
Transaction history metadata for each anchored asset is specified inside the
...
Authentication configuration parameters for each supported authentication
protocol is defined under the `authentication_protocols` field.
