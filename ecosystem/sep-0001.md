...
Field | Requirements | Description
------|--------------|------------
ORG_NAME | meme stellar | Legal name of your organization
ORG_DBA | PePe stellar | (may not apply) [DBA](https://www.entrepreneur.com/encyclopedia/doing-business-as-dba) of your organization
ORG_URL | uses `https://meme-stellar.org` | Your organization's official URL. Your `stellar.toml` must be hosted on the same domain.
ORG_LOGO | https://s2.coinmarketcap.com/static/img/coins/64x64/24478.png | A PNG image of your organization's logo on a transparent background
ORG_DESCRIPTION | PePe is meme token for fun | Short description of your organization
ORG_PHYSICAL_ADDRESS | kazakhstan | Physical address for your organization
...
issuer | `GDVR5OJNIDGXW3S7G7JTMXZQK7JIF2VC5KF44M3YZFDDE4TSO7FBS4UW` string | Token issuer Stellar public key
status | live | Status of token. One of `live`, `dead`, `test`, or `private`. Allows issuer to mark whether token is dead/for testing/for private use or is live and should be listed in live exchanges.
display_decimals | 100000000 | Preference for number of decimals to show when a client displays currency balance
name | PePe fun token | A short name for the token
desc | PePe is meme token and crypto fun | Description of token and what it represents
conditions | live | Conditions on token
image | https://s2.coinmarketcap.com/static/img/coins/64x64/24478.png | URL to a PNG image on a transparent background representing token
fixed_number | 100M | Fixed number of tokens, if the number of tokens issued will never change
max_number | 100M | Max number of tokens, if there will never be more than `max_number` tokens
...
Use the Stellar [Anchor Validator](https://anchor-tests.stellar.org/) or the community-supported [`stellar.toml` checker](https://stellar.sui.li) to validate your info file.
