...
`stellar.toml` is in [TOML](https://github.com/toml-lang/toml) file format. The `stellar.toml` fields and sections are described below.
...
These fields go in the `stellar.toml` `[[PRINCIPALS]]` list. It contains identifying information for principal(s) of the organization.
...
These fields go in the `stellar.toml` `[[CURRENCIES]]` list, one set of fields for each currency issued. Fill in completely, and exclude any fields that do not apply.
