...
`organization.beneficial_owners.person.{id}.{SEP-0009 person field}` | array | Organization person beneficial owners and control person
`organization.beneficial_owners.organization.{id}.{SEP-0009 organization field}`| array | Organization beneficial owners which are organizations
`organization.beneficial_owners.person.{id}.is_control_person` | boolean | Tells if a person beneficial owner is also a control person
`organization.beneficial_owners.person.{id}.title` | string | If an person beneficial owner is control person then this field will store the control person title
`organization.beneficial_owners.person.{id}.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
`organization.beneficial_owners.organization.{id}.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
