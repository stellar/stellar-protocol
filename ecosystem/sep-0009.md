...
```
The dot notation is _not_ an indication that the fields described should be contained in a nested object under a top-level key. For example, the following would not be compliant:
```json
{
	"organization": {
