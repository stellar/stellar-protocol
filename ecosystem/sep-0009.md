...
To improve user's privacy and to distinguish between various levels of risk that VASPs (Virtual Asset Service Providers) need to define in thier KYC/AML policy the KYC information will be groupped into distinct KYC Levels. This way each VASP (issuers, Banks...) can request or expose a certain level of KYC information depending on the risk profile of the customer, risk of the transaction and the risk appetite of the VASP itself.

Note: some of the information marked as KYC2 level might be extracted from documents needed for KYC1 level (e.g. place of birth - might be on the ID document), but in the process of exchange information such information should not be exchanged between VASPs unless needed a higher level the KYC is needed for a certain operation. The main reason of using KYC level is to protect individual privacy and personal information. 
Note2: This document is not describing the obligations of all parties involved in exchange of personal information but they need to comply with privacy laws. (like GDPR - Europe)
