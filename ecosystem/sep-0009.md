...
`organization.beneficial_owners.person.{uniqueIdentifier}.{SEP-0009 person field}` | array | Organization person beneficial owners and control person
`organization.beneficial_owners.organization.{uniqueIdentifier}.{SEP-0009 organization field}`| array | Organization beneficial owners which are organizations
`organization.beneficial_owners.person.{uniqueIdentifier}.is_control_person` | boolean | Tells if a person beneficial owner is also a control person
`organization.beneficial_owners.person.{uniqueIdentifier}.title` | string | If an person beneficial owner is control person then this field will store the control person title
`organization.beneficial_owners.person.{uniqueIdentifier}.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
`organization.beneficial_owners.organization.{uniqueIdentifier}.ownership_percentage` | string | Holds the percentage owned by the beneficial owner. Must be a number in the range 0 - 100 with at most 2 decimal places
...
### Beneficial Owners
* Person Beneficial Owners - individual (person)  which holds ownership.
* Organization Beneficial Owner - entity (organization)  which holds ownership. 
* Control Person - Control persons are able to use their authority or influence to make decisions on the corporation's activities. Control persons include but are not limited to senior managers, members of the board of directors, and officers such as the CEO and CFO. A control person is also called an affiliated person.
