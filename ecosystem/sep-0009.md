...
These field names should always be used as strings. For example:
```json
{
	"organization.name": "Stellar Development Foundation"
}
```
The dot notation (`organization.<field>`) is _not_ an indication that the fields described should be nested under a top-level `"organization"` object.
