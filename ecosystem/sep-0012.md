...
`transaction_id` | `string` | (optional) Transacion id with which customer's info is associated. In cases when information required from the customer depends on the transaction (e.g. more infromation is required for bigger amounts), this field should be provided to indicate to the anchor transaction's context.                                                                                                                                               |
...
* `v1.12.0`: Add `transaction_id` field to `/customer` `GET` and `PUT` ([#1431](https://github.com/stellar/stellar-protocol/pull/1431))
