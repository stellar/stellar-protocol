...
```
GET [KYC_SERVER]/customer?account=<public_key>&memo=<memo>&memo_type=<memo-type>&type=<customer-type>
...
`account` | `G...` string | (optional) The Stellar account ID used to identify this user. This should be passed if the customer has not been registered.
`memo` | string | (optional) a properly formatted memo according to our [memo documentation](https://developers.stellar.org/docs/glossary/transactions/#memo).
