...
Client applications can use a single Stellar account to hold multiple customers' funds. To differentiate customers using the same Stellar account, the decoded SEP-10 JWT may contain `memo` and `memo_type` attributes. The anchor should use these attributes in addition to the Stellar account in their data model to identify customers. See the [SEP-10 Memos](sep-0010.md#memos) section for more information.
...
`memo` | string | (optional) a client-generated [memo](https://developers.stellar.org/docs/glossary/transactions/#memo) that uniquely identifies a customer. If a `memo` is present in the decoded SEP-10 JWT, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
`memo_type` | string | (optional) type of `memo`. One of `text`, `id` or `hash`. If `hash`, `memo` should be base64-encoded. If a `memo` is present in the decoded SEP-10 JWT, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
