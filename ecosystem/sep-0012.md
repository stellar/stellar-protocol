...
explicitly passed in the request body the server can still infer this information from the decoded SEP-10/SEP-45 JWT's
`sub` value. If a `PUT /customer` request has already been made for the customer, the client should preferably use the
`id` returned in the response instead.
...
Delete all personal information that the anchor has stored about a given customer. `[account]` is a Stellar account ID
