...
GET [KYC_SERVER]/customer?account=<public-key>&type=<customer-type>
GET [KYC_SERVER]/customer?account=<public-key>&memo=<memo>&type=<customer-type>
...
`account` | `G...` string | (**deprecated**, optional) This field should match the Stellar account authenticated via SEP-10.
`memo` | string | (optional) the client-generated [memo](https://developers.stellar.org/docs/glossary/transactions/#memo) that uniquely identifies the customer. If a `memo` is present in the decoded SEP-10 JWT, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
`memo_type` | string | (**deprecated**, optional) type of `memo`. One of `text`, `id` or `hash`. Deprecated because memos should always be of type `id`, although anchors should continue to support this parameter for outdated clients. If `hash`, `memo` should be base64-encoded. If a memo is present in the decoded SEP-10 JWT's `sub` value, can be ignored. See the [Shared Accounts](#shared-accounts) section for more information.
...
The client can always use the `account` and `memo` parameters to uniquely identify a customer. However, if a `PUT /customer` request has already been made for the customer, the client may use the `id` returned in the response instead.
...
`account` | `G...` string | (**deprecated**, optional) This field should match the Stellar account authenticated via SEP-10. If specified, `id` should not be specified.
`memo` | string | (optional) a client-generated [memo](https://developers.stellar.org/docs/glossary/transactions/#memo) that uniquely identifies a customer. If a memo is present in the decoded SEP-10 JWT's `sub` value, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
`memo_type` | string | (**deprecated**, optional) type of `memo`. One of `text`, `id` or `hash`. Deprecated because memos should always be of type `id`, although anchors should continue to support this parameter for outdated clients. If `hash`, `memo` should be base64-encoded. If a memo is present in the decoded SEP-10 JWT's `sub` value, this parameter can be ignored. See the [Shared Accounts](#shared-accounts) section for more information.
...
`account` | `G...` string | (**deprecated**, optional) This field should match the Stellar account authenticated via SEP-10.
`memo` | string | (optional) the client-generated [memo](https://developers.stellar.org/docs/glossary/transactions/#memo) that uniquely identifies the customer. If a `memo` is present in the decoded SEP-10 JWT, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
`memo_type` | string | (optional) type of `memo`. One of `text`, `id` or `hash`. If `hash`, `memo` should be base64-encoded. If a `memo` is present in the decoded SEP-10 JWT, it must match this parameter value. See the [Shared Accounts](#shared-accounts) section for more information.
