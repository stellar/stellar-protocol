...
Client applications can use a single Stellar account to hold multiple users' funds. To distinguish users that use the same Stellar account, the decoded SEP-10 JWT's `sub` value may also contain a memo value. The anchor should use this attributes in addition to the Stellar account in their data model to identify users. 
