...
This SEP also supports authenticating users of shared, omnibus, or pooled Stellar accounts. Clients can use [memos](#memos) or [muxed accounts](../core/cap-0027.md) to distinguish users of these accounts.
...
- A **Client**: the software used by the holder of the account being authenticated by the **Server**.
    - The account being authenticated is the **Client Account**. This can be a Stellar account (`G...`) or a muxed account (`M...`).
- A **Client Domain** (optional): a domain hosting a [SEP-1 stellar.toml](sep-0001.md) containing a `SIGNING_KEY` used for [Verifying the Client Domain](#verifying-the-client-domain)
...
1. The **Client** requests a unique [`challenge`](#challenge) transaction from the **Server**, which is represented as specially formed Stellar transaction
1. If the request contains a `client_domain` parameter, the **Server** may fetch the **Client Domain Account** and generate the challenge transaction with an additional Manage Data operation as described in the [Response](#response) section.
...
1. If the transaction has a Manage Data operation with key `client_domain`, the **Server** verifies that the source account of the operation signed the transaction and includes an additional `client_domain` claim in the JWT included in the response
...
`memo` | string | (optional) The memo of type `id` to attach to the challenge transaction. Only permitted if a muxed account (`M...`) is not used. `hash` and `text` memos are not supported. See the [Memo](#memos) section for details.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * memo: derived from the `memo` value passed by the **Client** if specified, omitted otherwise
  * operations:
    ...
    * subsequent operations, order unimportant:
      ...
      * (optional) `manage_data(source: client domain account, key: 'client_domain', value: client_domain)`
        ...
        * Add this operation if the server supports [Verifying the Client Domain](#verifying-the-client-domain) and the client provided a `client_domain` parameter in the request.
...
* `sub` (the principal that is the subject of the JWT, [RFC7519, Section 4.1.2](https://tools.ietf.org/html/rfc7519#section-4.1.2)) — there are three possible formats:
  * If the **Client Account** is a muxed account, the `sub` value should also be the muxed account (`M...`).
  * If a memo was attached to the challenge transaction, the `sub` should be the Stellar account appended with the memo, separated by a colon (`G...:17509749319012223907`).
  * If neither of the above conditions are true, the `sub` value should be Stellar account (`G...`).
...
The `memo` parameter supported in `GET <WEB_AUTH_ENDPOINT>` API calls can be used to communicate to the **Server** that the client intends to authenticate a Stellar account with an additional claim that the account is shared and that the user should be identified using the `account` and `memo` parameters, instead of only using `account`. The value of the `memo` passed will ultimately be added to the decoded JWT's `sub` field, separated from the account address by a colon (`:`).

Note that conceptually, a Stellar account and memo (of type `id`) is equivalent to a muxed account (`M...`) defined by [CAP-0027](../core/cap-0027.md). If a SEP-10 implementation supports the use of memos to identify users of shared accounts, it is highly recommended to also support the muxed account address format. **Muxed accounts will become the primary (and eventually exclusive) method for identifying a user of a shared, omnibus, or pooled account.**
