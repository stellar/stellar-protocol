...
* `memo` — (optional) the memo provided in the challenge, always of type `string` even when `memo_type` is equal to `id` (unsigned 64-bit integer) because JSON and some runtimes do not support 64-bit integers.
