...
`memo` | string | (optional) The memo to attach to the challenge transaction, for `hash` this should be base64-encoded.
...
* `memo_type` — (optional) the memo type provided in the challenge, one of `text`, `id` or `hash`
...
Stellar transactions may be specified with a memo and services that share a single account for many users typically use the memo as a method to distinguishing one user from another, or one internal off-network account from another.

The `memo_type` and `memo` parameters supported in `GET <WEB_AUTH_ENDPOINT>` API calls can be used to communicate to the server that the client intends to authenticate a Stellar account with an additional claim that the authenticating user has control of or is possession of the provided `memo`.
