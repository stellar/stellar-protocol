...
- `iat` (the time at which the JWT was issued
  [RFC7519, Section 4.1.6](https://tools.ietf.org/html/rfc7519#section-4.1.6)) — current timestamp (for example `1530644093`)
...
- `url` full URL of the request, including scheme, path and query parameters.
...
- If `client_domain` is specified, the token must be signed with the `SIGNING_KEY`. (See
  [Verifying the Client Domain](#verifying-the-client-domain) section for more information)
...
```
eyJhbGciOiJFZERTQSJ9.eyJqdGkiOiIzYjExYTRhYS1lMmE4LTRkYTUtYTUxOC1iMDU5YWMwMjUwOGYiLCJpYXQiOjE3MTA5NzY2MDcsImV4cCI6MTAzNTA5NzY2MDcsInVybCI6Imh0dHBzOi8vYXV0aC5leGFtcGxlLmNvbS8_YWNjb3VudD1HQ0lCVUNHUE9IV01NTUZQRlREV0JTVkhRUlQ0RElCSjdBRDZCWkpZRElUQksyTENWQllXN0hVUSZjbGllbnRfZG9tYWluPWV4YW1wbGUtd2FsbGV0LnN0ZWxsYXIub3JnIn0.FhX1sxGZcFMEwfRE4W3fe8uB5PUtBBS3_nkKUVHx9Ulw3Kc3OvJ5bwLRXuWRpBndvuK2S-yKdKBUCLqd4QO-Cg
```

This token was signed with the `GCADAIACE6CRWGOB3HJRXIOHMQEUKPNLAUIYFIE26F3OK72RSHGAMARK` key, belonging to a
hypothetical domain `example-wallet.stellar.org`

Another example of the valid signed token:

```
eyJhbGciOiJFZERTQSJ9.eyJqdGkiOiI1YWExYTNhYi0zY2Q2LTRkYWYtODQyMi0xYmI5Zjc1NTI1YjEiLCJpYXQiOjE3MTA5NzY5MDYsImV4cCI6MTAzNTA5NzY5MDYsInVybCI6Imh0dHBzOi8vYXV0aC5leGFtcGxlLmNvbS8_YWNjb3VudD1HQ1hYSDZBWUpVVlRER0lIVDQyT1pOTUYzTEhDVjRET0tDWDZISERLV0VDVVpZWERaU1daTjZIUyZtZW1vPTEyMzQ1NjcifQ.Gi7aPIwLBss54DJzKhi9lRlUViCkLbbjT0SQFrG9eGGt7TAGD1VeLfj2LFBCOoCFytXWDufplY4iJk4XG_jwDw
```
...
token. Next, server must verify that:

1. URL's path must match `<WEB_AUTH_ENDPOINT>`
2. URL's query parameters must match request's parameters. Server should address possible permutations of the query
   parameters.
...
- `v3.4.0`: Add Authorization header for `GET  <WEB_AUTH_ENDPOINT>` ([#1470](https://github.com/stellar/stellar-protocol/pull/1470))
