...
This SEP also supports authenticating clients that share a Stellar account and use [memos](#memos) to distinguish users.
...
`memo` | string | (optional) The memo of type `id` to attach to the challenge transaction. `hash` and `text` memos are not supported. See the [Memo](#memos) section for details.
...
* `sub` (the principal that is the subject of the JWT, [RFC7519, Section 4.1.2](https://tools.ietf.org/html/rfc7519#section-4.1.2)) — the public key of the authenticating Stellar account (`G...`). If the challenge included a memo, it should appended to the `sub` string separated by a colon. For example, `G...:17509749319012223907`.
