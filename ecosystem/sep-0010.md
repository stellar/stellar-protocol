...
1. The server verifies the weight provided by the signers meet the required threshold(s)
1. The server responds with a [JWT](https://jwt.io) that represents the user's session and includes the weight and thresholds
...
Older version of SEP-10 did not include the `thr` and `wht` and if the claims are not present the service can assume all thresholds were met.
