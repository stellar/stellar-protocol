...
1. The client verifies that the transaction has exactly one Manage Data operation containing the user's account and a nonce. This operation must be the first operation. The client ignores the home domain included.
1. The client verifies that the transaction has at most one Manage Data operation with key `server_hostname` with the server's account as the source account. This operation may appear in any position after the first operation. If present, the client verifies that this operation contains the hostname of the SEP-10 server from which the client requested the challenge.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * operations:
    ...
    * subsequent operations, order unimportant:
      * `manage_data(source: server_account, key: 'server_hostname', value: server_hostname)`
        * The value is the fully qualified domain name of the SEP-10 server that issued the challenge transaction. It can be at most 64 characters.
