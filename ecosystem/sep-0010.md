...
If a client is interacting directly with a SEP-10 server without using a stellar.toml, it must still verify the home domain to ensure it only signs challenges issued by the server that the client intends to authenticate.
