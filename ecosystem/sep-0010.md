...
* verify that client signatures provide a weight\* respective to their thresholds required to prove a level of control the service needs;
...
When determining the weights of the signers the servers signer should be excluded, even if it is a signer on the account. In most cases the server should verify that the signers provide full control of the account by checking that the sum of the weights is equal or greater than all thresholds, low, mid and high.
...
For servers that issue JWTs for signers that have less than full control, the JWT should contain additional claims that capture the level of control the user has over the account.
...
If the SEP-10 server issues JWTs with varying levels of control over the account then services consuming the JWT may need to check that the value in `wht` is equal or greater than a value in `thr.low`, `thr.med`, and/or `thr.high`.
