...
An account's master key may meet not meet any threshold of control or could have had its weight reduced to zero. Most applications should not assume possession of the master key is possession of an account.

An account's signers may include third-party services providing services. Authenticating users with less than any threshold may allow a third-party to authenticate as a user.
...
The server should only issue a JWT if the appropriate thresholds are met. If a server is supporting a variety of applications it may choose to use additional application specific claims to capture the threshold of control the user has proven.

### Funds Movement Signing Authority

A server that needs to verify that a user has authority that is aligned with the capability to move money out of an account, such as an anchor implementing [SEP-24] that will let the user define the destination of withdrawn funds, can verify that the user has signing authority to move funds by checking that they meet the medium threshold. It should do this by checking that the sum of the weights of the challenge transaction signers is equal or greater to the medium threshold.
...
A server may choose to issue JWTs for less than all thresholds and based on any other application specific logic. It's important to keep in mind that a Stellar account may have third-parties who are signers and authenticating users with less than any threshold may allow a third-party to authenticate as a user.
