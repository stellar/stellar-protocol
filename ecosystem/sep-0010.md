...
1. The client signs the transaction using the secret key(s) of signers of their account
...
* The server can choose the required threshold(s) that the user needs on the account, if any
* The server can choose to include other application specific claims
* Other servers that consume the JWT can choose the required threshold(s) or application specific claims a user needs on the account for specific functionality
...
Servers may add claims to provide information about the state of the account, threshold of control, or signers that the user is in possession:
* `acx` — boolean indicating if the account exists
* `sig` — array of public keys of the signers of the challenge transaction
* `thr`
...
Servers may add other claims specific to your application, see [RFC7519].
...
Services that consume the JWT for authentication and authorization need to validate the JWT. General [JWT best practices](#jwt-best-practices) should be followed for validating the claims within the header and payload.

Services that use the JWT as proof the user meets a threshold of control of a Stellar account should check that the threshold (`thr`) and weight (`wht`) claims are present and that they meet the threshold(s) required by the service. Services may choose to check that the signers in the JWT and their weights still meet requirements the service has by checking the state of the account on the Stellar network. If an account does not exist a server should check that the signer (`sig`) is the account.

Services may optionally use the list of signers (`sig`) as authorization for providing specific signers with access to specific use cases.
