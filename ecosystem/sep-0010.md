...
Notes:
1. <a name="note1"></a>Some use cases of SEP-10 may not involve a home domain hosting a stellar.toml. If a client is connecting directly to a SEP-10 serverit must still verify the home domain to ensure it only signs challenges issued by the server that the client intends to authenticate.
