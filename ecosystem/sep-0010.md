...
This SEP also supports authenticating clients that share a Stellar account and use [memos](#memos) to distinguish users or accounts for payments.
...
----------|---------------|------------
`account` | `G...` string | The stellar account that the **Client** wishes to authenticate with the **Server** 
`memo` | string | (optional) The memo to attach to the challenge transaction, for `hash` this should be base64-encoded. See the [Memo](#memos) section for details.
`memo_type` | string | (optional) The type of memo to attach to transaction, one of `text`, `id` or `hash`.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * memo: derived from the `memo_type` and `memo` value passed by the **Client** if specified, omitted otherwise
...
* `memo_type` — (optional) the memo type provided in the challenge, one of `text`, `id` or `hash`. See the [Memos](#memos) section for more information.
* `memo` — (optional) the memo provided in the challenge, always of type `string` (even when `memo_type` is equal to `id`) to support parsing the value in languages that don't support big numbers.
...
Stellar transaction memos are used for a variety of purposes in the ecosystem, but in the context of this standard memos attached to challenge transactions are typically used to distinguish users who share a single Stellar account. This is especially relevant for applications who custody user funds in shared Stellar accounts.

The `memo_type` and `memo` parameters supported in `GET <WEB_AUTH_ENDPOINT>` API calls can be used to communicate to the **Server** that the client intends to authenticate a Stellar account with an additional claim that the account is shared and that the user should be identified using the `account` and `memo` parameters, instead of only using `account`.
