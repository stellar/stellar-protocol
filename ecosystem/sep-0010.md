...
1. The client signs the transaction using keys that control their Stellar account
1. The client submits the signed challenge back to the server using [`token`](#token) endpoint
1. The server looks up the signers for the account
1. The server verifies the signatures
1. The server compares the weight provided by the signatures with the thresholds of the account against a minimum level of control required
1. If the signatures provide the minimum level of control, the server responds with a [JWT](https://jwt.io) that represents the user's session and includes the level of control the user has over the account
...
* The server can verify that the client holds the secret key(s) with control of an account
...
* The server can choose a minimum level of control a user needs of the account
* Other servers can choose specific levels of control a user needs of the account for different operations
...
* verify that client signatures provide a minimum weight\* required when compared to the account's thresholds that is appropriate for the level of control the service needs;
...
\* It is better to fail authentication if the weight provided by the signatures will not provide sufficient control for any service that will consume the JWT. In most cases a service needs proof of complete control of a Stellar account and the server should verify that the sum of the weights of the signers who signed the transaction is equal or greater than all thresholds (low, mid, and high). When summing the weights the servers signer should be excluded, even if it is a signer. 
...
* `wgt` — weight of the signers who signed the challenge transaction, excluding the weight of the server signer if it is also a signer of the account
...
Services that use the JWT as proof of control of a Stellar account need to validate the JWT. General [JWT best practices](#jwt-best-practices) should be followed for validating the claims within the header and payload. In additional services should validate that the weight (`wgt`), in respect to the account's thresholds (`thr`), provides a sufficient level of control for the operations the service is allowing the user to perform.

If the SEP-10 server only issues JWTs if the signing weight is above all account thresholds then no additional validation will be necessary.

If the SEP-10 server issues JWTs with varying levels of control over the account then services consuming the JWT may need to check that the value in `wgt` is equal or greater than a value in `thr.low`, `thr.med`, and/or `thr.high`.
