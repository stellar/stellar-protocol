...
* verify that transaction envelope has signatures from valid signers of the transaction's source account with summed weight greater than the medium threshold on the transaction's source account;
* verify that transaction envelope has signatures from valid signers of the operation's source account with summed weight greater than the medium threshold on the operation's source account;
