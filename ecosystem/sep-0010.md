...
Services that use the JWT as proof the user meets a threshold of control of a Stellar account need to validate the JWT. General [JWT best practices](#jwt-best-practices) should be followed for validating the claims within the header and payload and the service must check that the threshold (`thr`) and weight (`wht`) claims are present and that they meet the threshold(s) required by the service.
