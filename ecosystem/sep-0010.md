...
- `host` domain of the server to which the request is being made for. 
- `path` the path of the request (`<WEB_AUTH_ENDPOINT>`)
...
```
eyJhbGciOiJFZERTQSJ9.eyJpYXQiOjE3MTEwNzMyMjAsImV4cCI6MTcxMTA3NDEyMCwiYWNjb3VudCI6IkdDNlVDWFZUQU1ORzVKTE9NWkJTQ05ZWFZTTk5GSEwyM1NKUFlPT0ZKRTJBVllERFMyRkZUNDVDIiwiY2xpZW50X2RvbWFpbiI6ImV4YW1wbGUtd2FsbGV0LnN0ZWxsYXIub3JnIiwiaG9zdCI6ImV4YW1wbGUuY29tIiwicGF0aCI6Ii9zZXAxMC9hdXRoIn0.SJwDQBr_xjvb-Nx23t8xhnxzVZVY_IKCzTtPk0-xHfkse7sIyJK_Aobzq3JYpxZ5F5CpNEfOi4Yu5U4mDWVnDA
```

This token was issued for the request URL `https://example.com/sep10/auth?account=GC6UCXVTAMNG5JLOMZBSCNYXVSNNFHL23SJPYOOFJE2AVYDDS2FFT45C&client_domain=example-wallet.stellar.org` and signed with the `GCADAIACE6CRWGOB3HJRXIOHMQEUKPNLAUIYFIE26F3OK72RSHGAMARK` key, belonging to a
hypothetical domain `example-wallet.stellar.org`

Another example of the valid signed token:

```
eyJhbGciOiJFZERTQSJ9.eyJpYXQiOjE3MTEwNzMyMjMsImV4cCI6MTcxMTA3NDEyMywiYWNjb3VudCI6IkdDWFhINkFZSlVWVERHSUhUNDJPWk5NRjNMSENWNERPS0NYNkhIREtXRUNVWllYRFpTV1pONkhTIiwibWVtbyI6IjEyMzQ1NjciLCJob3N0IjoiZXhhbXBsZS5jb20iLCJwYXRoIjoiL3NlcDEwL2F1dGgifQ.wQdzjK2iPiU2C1rpreOEgZalwAdo1nxs2cnD6N1bst4bpwHTnbBE1jU4un0cJjWBMkH_NX3J8gfX082h6VMKDw
```

The token was issued for the request URL `https://example.com/sep10/auth?account=GCXXH6AYJUVTDGIHT42OZNMF3LHCV4DOKCX6HHDKWECUZYXDZSWZN6HS&memo=1234567`. 
...
1. `host` must match server's domain
2. `path` path must match `<WEB_AUTH_ENDPOINT>`
3. Query parameters from claims must match request's parameters. 
