...
`memo` | string | (optional) The value of memo to attach to transaction, for `hash` this should be base64-encoded.
...
* `transaction`: an XDR-encoded Stellar transaction with the following:
  ...
  * memo derived from the `memo_type` and `memo` value passed by the client, if specified
...
* use operations's source account, and memo if set, to determine the authenticating client and perform any additional service-specific validations.
...
* `memo` — (optional) the memo provided in the challenge, always of type `string` (even when `memo_type` is equal `id`) to support parsing the value in languages that don't support big numbers.
