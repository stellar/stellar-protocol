...
* verify that transaction envelope has signatures from valid signers of the operation's source account. If the account does not exist on the public Stellar network, then a signature by the operation source must be attached to the transactions. Otherwise, if the account does exist on the public Stellar network, signatures must be present on the transaction with summed weight greater than the medium threshold on the operation's source account;
