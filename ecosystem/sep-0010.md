...
1. The client verifies that the transaction is signed by the `SIGNING_KEY` obtained through discovery flow.
  * If this verification fails, client should re-execute the discovery flow and attempts to resume the authentication flow with the same challenge and a freshly retrieved `SIGNING_KEY`. This is necessary to support seamless key rotation for web services. If verification fails the second time, client must not continue the authentication and should present to the user (or write into the log, in case of machine-to-machine authentication) a warning about potential spoofing attack.
