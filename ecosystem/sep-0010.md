...
* if the operations source account does not exist:
  ...
  * verify thta client signature is correct for the master key of the account;
...
Services that use the JWT as proof the user meets a threshold of control of a Stellar account must check that the threshold (`thr`) and weight (`wht`) claims are present and that they meet the threshold(s) required by the service.

Services may optionally use the list of signrs (`sig`) as authorization for providing specific signers with access to specific use cases.
