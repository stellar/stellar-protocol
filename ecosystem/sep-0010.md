...
1. The client verifies that the transaction has a single Manage Data operation with its source account set to the user's account and value set to a nonce value. The client verifies the home domain included is the domain from the first step.<sup>[1](#note1)</sup>
...
Notes:
1. <a name="note1"></a>Some use cases of SEP-10 may not involve a home domain hosting a stellar.toml. If a client is connecting directly to a SEP-10 server it must still verify the home domain to ensure it only signs challenges issued by the server that the client intends to authenticate.
