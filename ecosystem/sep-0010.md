...
Services that use the JWT as proof the user meets a threshold of control of a Stellar account should check that the threshold (`thr`) and weight (`wht`) claims are present and that they meet the threshold(s) required by the service. If an account does not exist a server should check that the signer (`sig`) is the account.
