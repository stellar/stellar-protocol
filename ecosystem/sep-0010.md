...
Note that conceptually, a Stellar account and memo (of type `id`) is equivalent to a muxed account (`M...`) defined in the protocol by [CAP-0027](../core/cap-0027.md) and standardized for use in the ecosystem in [SEP-0023](sep-0023.md). If a SEP-10 implementation supports the use of memos to identify users of shared accounts, it is highly recommended to also support the muxed account address format. Muxed accounts will become the primary (and eventually exclusive) method for identifying a user of a shared, omnibus, or pooled account.
