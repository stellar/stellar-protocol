...
* verify that transaction envelope has signatures corresponding to whichever roles are to be included in the token (see the [roles section](#claims) for more info on roles).
...
#### Optional Claims
The JWT may also contain the following optional claims:

* `roles`
The JWT may optionally include a `roles` (array) claim. In the case that there is no `roles` claim, consumers should assume the roles are equivalent to `['Master']` (see below). `roles` are permission claims related to the **operation's** source account. The available roles are:
  1. `Master` - The JWT requestor signed with the account's master key (this says nothing about the weight of the master key relative to any of the account's thresholds or whether or not the account even exists on the Stellar network at all).
  2. `Low` - The JWT requestor signed with signatures summing to equal or greater than the account's low threshold.
  3. `Medium` - The JWT requestor signed with signatures summing to equal or greater than the account's medium threshold.
  4. `High` - The JWT requestor signed with signatures summing to equal or greater than the account's high threshold.
