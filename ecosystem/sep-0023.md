...
signed payload signers (introduced in [CAP-40]), contracts, liquidity pools,
claimable balances, and muxed contracts (introduced in [CAP-84]).
...
| STRKEY_MUXED_CONTRACT    | 22 << 3    | W          | yes   | Hash |
...
3.  If we are encoding a multiplexed address (a muxed account or a muxed
    contract), append an 8-byte ID in network byte order (most significant
    byte first). For a muxed contract the key bytes in the prior step are the
    32-byte contract ID, so the payload is 40 bytes, the same as a muxed
    account.
...
1. Valid muxed contract address in which unsigned id exceeds maximum signed
   64-bit integer
...
1. The unused trailing bit must be zero in the encoding of the last three bytes
   (24 bits) as five base-32 symbols (25 bits) of a muxed contract

   - Strkey:
     `WA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAAWWD`

1. Invalid length (congruent to 6 mod 8) of a muxed contract

   - Strkey:
     `WA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAAWWCA`

1. Invalid length (base-32 decoding should yield 43 bytes, not 44) of a muxed
   contract

   - Strkey:
     `WA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAAAIOUI`

1. Invalid algorithm (low 3 bits of version byte are 7) of a muxed contract

   - Strkey:
     `W47QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAADXHW`

1. Invalid checksum of a muxed contract
