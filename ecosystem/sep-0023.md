...
revelation of a SHA-256 preimage), muxed accounts (introduced in [CAP-27]),
signed payload signers (introduced in [CAP-40]), and muxed contracts
(introduced in [CAP-84]).
...
| STRKEY_MUXED_CONTRACT    | 13 << 3    | N          | yes   | Hash |
...
3.  If we are encoding a multiplexed address&mdash;a muxed account (`M...`) or
    a muxed contract (`N...`)&mdash;append an 8-byte identifier in network byte
    order (most significant byte first). For a muxed account this identifier is
    the memo ID; for a muxed contract it is the multiplexing identifier.
...
1. Valid muxed contract

   - Strkey
     `NA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAA3N6`
   - type: `HASH_TYPE_SHA256`
   - id: 0
...
1. Valid muxed contract in which unsigned id exceeds maximum signed 64-bit
   integer

   - Strkey
     `NA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJVAAAAAAAAAAAAC3SE`
   - type: `HASH_TYPE_SHA256`
   - id: 9223372036854775808
   - contract: `CA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUWDA`
   - hash:
...
1. Muxed contract with an unused trailing bit that is not zero

   - Strkey:
     `NA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAA3N7`

1. Muxed contract with invalid algorithm (low 3 bits of version byte are 7)

   - Strkey:
     `N47QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAD24K`

1. Muxed contract with invalid checksum

   - Strkey:
     `NA7QYNF7SOWQ3GLR2BGMZEHXAVIRZA4KVWLTJJFC7MGXUA74P7UJUAAAAAAAAAAAAA3NC`
