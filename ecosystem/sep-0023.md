...
signing authority upon revelation of a SHA-256 preimage), muxed accounts
(introduced in [CAP-27]), and signed payload signers (introduced in
[CAP-40]).
...
5.  Compute a 16-bit CRC16 checksum of the result of the prior step
    (using polynomial x<sup>16</sup> + x<sup>12</sup> + x<sup>5</sup>
    \+ 1). Append the two-byte checksum to the result of the previous
    step (e.g., producing a 35-byte quantity for a non-multiplexed
    ED25519 public key, or 43 byte quantity for a multiplexed one).

6.  Encode the result of the previous step using [RFC4648 base-32
...
The CRC is an error detection mechanism but does not guarantee that a
strkey has not been truncated. Existing strkeys have a determinible
length based on their strkey type and algorithmn specifier.
...
1. Valid signed payload with an ed25519 public key and a 32-byte
   payload.
