...
If `status` is `pending_receiver`, the Receiving Anchor is processing the information sent and determining if the transaction can proceed. In this case, `stellar_account_id`, `stellar_memo`, & `stellar_memo_type` will not be present, and the Sending Anchor should monitor the transaction's status until it moves to `pending_sender` or `error`. Sending Anchors can monitor the transaction's status by registering a callback URL via [`PUT /transactions/:id/callback`] or by polling [`GET /transactions/:id`]. When the transaction status moves to `pending_sender`, the Stellar account & memo fields should be populated.
