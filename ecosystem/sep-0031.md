...
```json
{
   "receive":{
      "USD":{
         ...
         "sep12_types": {
          "sep31_sender": {
            "description": "Customers of this type must be U.S. citizens and are limited to sending payments of less than $10,000"
          },
          "sep31_receiver": {
            "description": "Customers of this type must be U.S. citizens."
          },
          "sep31_large_sender": {
            "description": "Customers of this type must be U.S. citizens and do not have sending limits"
          },
          "sep31_foreign_receiver": {
            "description": "Customers of this type are non-U.S. citizens."
          }
         },
         "fields":{
            "transaction":{
...
* `sender_sep12_type`: (optional) the default value of the `type` parameter used in `SEP-12 GET /customer` requests for customers sending funds. This field can be omitted from the response if no KYC is necessary.
* `receiver_sep12_type`: (optional) the default value of the `type` parameter used in `SEP-12 GET /customer` requests for customers receiving funds. This field can be omitted if no KYC is necessary. Other values 
* `sep12_types`: (optional) an object containing the accepted values for the `type` parameter in [SEP-12](sep-0012.md) requests. Each key should map to an object value with a human-readable `description`. This field can be omitted from the response if no KYC is necessary.
...
If `sender_sep12_type` or `receiver_sep12_type` are present in the response, the client must register the sender and receiver via [SEP-12](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-get) using one of the values described in `sep12_types`. The receiving anchor uses this parameter to know what information needs to be collected on the customer.
