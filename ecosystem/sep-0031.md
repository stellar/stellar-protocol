...
`amount_in` | string | (optional) The amount of the Stellar asset received or to be received by the Receiving Anchor. It should be equals to the `amount` value used in the [`POST /transactions`](#post-transactions) request. If a `quote_id` was provided in the [`POST /transactions`](#post-transactions) request, the quote's `sell_amount` should also be equals to `amount_in`. </br></br>The Receiving Anchor may extract fees from this amount before converting it to the amount_out.
...
`amount_out` | string | (optional) The amount sent or to be sent by the Receiving Anchor to the Receiving Client. When using a `quote_id`, `amount_out` should always be the same as the quote's `buy_amount`. When using a `destination_asset` in the [`POST /transactions`](#post-transactions) request, it's expected that this value is only populated after the Receiving Anchor receives the incoming payment.
`amount_out_asset` | string | (optional) The asset delivered to the Receiving Client. Must be present if `quote_id` or `destination_asset` was included in the `POST /transactions` request. If `destination_asset` was used when creating the transaction, `amount_out_asset` should be equals to `destination_asset`. </br></br>The value must be in [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format).
`amount_fee` | string | (optional) The amount of fee charged by the Receiving Anchor. This value will be extracted from `amount_in` before converting it to `amount_out`. If a `quote_id` was used, fees should be described in the quote's `price_details` field.
`amount_fee_asset` | string | (optional) The asset in which fees are calculated in. Must be present if `destination_asset` was included in the `POST /transactions` request and the value must be in [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format). If a `quote_id` was used, fees and fee assets should be described in the quote's `price_details` field.
...
```js
post_transaction.amount = get_transaction.amount_in
```
...
#### Extra Amount Formulas (When Using Quotes)
...
```js
total fees = sum(quote.price_details)
```
...
* `v1.6.0`: Clarify how the SEP-31 amounts and fees relate to SEP-38 amounts and fees. ([#1199](https://github.com/stellar/stellar-protocol/pull/1199))
