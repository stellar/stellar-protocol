...
```json
{
  "receive":{
    "USD":{
      ...
      "min_amount":0.1,
      "max_amount":1000,
      "sender_sep12_required": true,
      "receiver_sep12_required": true,
      "sender_sep12_types": {
        "sep31-sender": {
...
* `receiver_sep12_required`: a boolean value indicating whether or not the receiving anchor requires the recieving client to be registered via SEP-12. If `true`, the sending anchor should use the appropriate value from `receiver_sep12_types` in a [SEP-12 GET /customer?type=](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-get) request to determine which fields should be collected and passed back via a [SEP-12 PUT /customer](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-put) request.
* `sender_sep12_required`: a boolean value indicating whether or not the receiving anchor requires the sending client to be registered via SEP-12. If `true`, the sending anchor should use the appropriate value from `sender_sep12_types` in a [SEP-12 GET /customer?type=](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-get) request to determine which fields should be collected and passed back via a [SEP-12 PUT /customer](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-put) request.
* `sender_sep12_types`: (optional) an object containing the accepted values for the `type` parameter in [SEP-12](sep-0012.md) requests for sending clients. Each key should map to an object value with a human-readable `description`. This field can be omitted from the response if no KYC is necessary for both the sender and receiver.
* `receiver_sep12_types`: (optional) an object containing the accepted values for the `type` parameter in [SEP-12](sep-0012.md) requests for receiving clients. Each key should map to an object value with a human-readable `description`. This field can be omitted from the response if no KYC is necessary for both the sender and receiver.
...
* `sender_sep12_type`: (**deprecated**, optional) value of the `type` parameter the sending anchor should use for a `SEP-12 GET /customer` request. This field can be omitted if no KYC is necessary.
* `receiver_sep12_type`: (**deprecated**, optional) value of the `type` parameter the sending anchor should use for a `SEP-12 GET /customer` request. This field can be omitted if no KYC is necessary.
...
If KYC is required for customers in some cases but not others, it is recommended to provide a `_sep12_type` value for all cases and return an empty `fields` object from `SEP-12 GET /customer` for the cases where no KYC is necessary.
...
`type` | string | (optional) A string for the `type` URL argument the sending anchor should use when making the `GET /customer` request. The value should be included in the `sep12_types` response object from `/info`.
