...
This protocol can be used if the Sending and Receiving Client wish to send and receive the same asset, but this is typically not the case. It is more common for the Sending Client to provide the Sending Anchor with a particular asset (the source asset) and have the Receiving Client receive a different asset (the destination asset).

In this case, the source asset must convert into the receiving asset at some point prior to delivering funds to the Receiving Client. This conversion must be done by either the Sending Anchor, the Stellar Network, or the Receiving Anchor. The subsections below outline the stratgies used for each method.

### Sending Anchor Asset Conversion

The Sending Anchor can collect some amount of the source asset from the Sending Client and send a different amount of the destination asset to the Receiving Anchor. 

This approach requires the Sending Anchor to provide an exchange rate, or quote, to the Sending Client. It also requies the Sending Anchor to hold a balance of the destination asset on Stellar. The Sending Anchor would use Payment operations to send the quoted amount of the destination asset to the Receiving Anchor.

### Stellar Network Asset Conversion

Instead of holding a balance of the destination asset on Stellar, the Sending Anchor can instead us Path Payments to convert the source asset to the destination asset on the Stellar Network. 

This approach requires a sufficiently liquid market to exist between the source and destination assets on the Stellar Decentralized Exchange (SDEX). Note that the exchange rate between the two assets is dynamic due to market fluctuations on the SDEX.
...
This requires the Receiving Anchor to implement the [SEP-38 Anchor RFQ API](sep-0038.md), which enables the Receiving Anchor to provide quotes to the Sending Anchor. These quotes can be [indicative](https://www.investopedia.com/terms/i/indicativequote.asp) or [firm](https://www.investopedia.com/terms/f/firmquote.asp). Sending Clients, and by extension the Sending Anchor, will naturally prefer to receive firm quotes, but the Receiving Anchor may not want to take on the risk involved.
...
The JSON object contains an entry for each currency that the Receiving Anchor can use to deliver payments to Receiving Clients. If the Receiving Anchor supports [SEP-38](sep-0038.md), the Sending Client can check the [SEP-38 GET /info](sep-0038.md#get-info) and [SEP-38 GET /prices](sep-0038.md#get-prices) endpoints to determine which Stellar assets the Receiving Anchor is willing to accept.
...
The following is an example request body if the Sending Anchor requested an _indicative_ quote from [`SEP-38 GET /price`](sep-0038.md#get-price) and found the _estimated_ rate acceptable. Note that the amount delivered to the Receiving Client in this scenario varies depending on the exchange rate used by the Receiving Anchor, which may differ from the rate provided in the `GET /price` response.
...

{
  ...
  "source_asset_issuer": "GDRHDSTZ4PK6VI3WL224XBJFEB6CUXQESTQPXYIB3KGITRLL7XVE4NWV",
...
Name | Type | Description
-----|-----|------
`amount` | number | (optional) Amount of payment in destination currency.
...
`source_amount` | number | (optional) Amount of the source asset the Sending Anchor will send to the Receiving Anchor. This attribute should only be used when the Sending Client checked the Receiving Anchors [SEP-38 GET /price](sep-0038.md#get-price) endpoint and found the estimated rate acceptable.
`source_asset_code` | string | (optional) The Stellar asset code of the asset the Sending Anchor will send. This asset must be listed in the [SEP-38 GET /info](sep-0038.md#get-info) response.
`source_asset_issuer` | string | (optional) The issuer of the Stellar asset the Sending Anchor will send. This asset must be listed in the [SEP-38 GET /info](sep-0038.md#get-info) response.
`quote_id` | string | The `id` returned from a `SEP-38 POST /quote` response. If this attribute is specified, none of the attributes listed above should also be included in the request.
