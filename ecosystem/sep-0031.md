...
`amount_in` | string | (optional) The amount of the Stellar asset received or to be received by the Receiving Anchor. Excludes any fees charged after Receiving Anchor receives the funds. It should be equals to the `amount` value used in the [`POST /transactions`](#post-transactions) request. Should be equals to `quote.sell_amount` if a `quote_id` was used.
...
```js
amount_out = amount_in - amount_fee - refunds.amount_refunded - refunds.amount_fee  // when `quote_id` is used, the price used to calculate the conversion between amounts is `quote.price` and not `quote.effective_price`.
```
