...
Allows an anchor to communicate basic info about what currencies their `SEND_SERVER` supports receiving from partner anchors.
...
```json
{
  "send": {
    "USD": {
      ...
      "fields": {
        "email_address" : {
          "description": "recipient's email address for transaction status updates (note that the senders email address will be handled by the sending anchor and doesn't need to be passed)"
        },
        "amount" : {
          "description": "amount in USD that you plan to send"
        },
        "routing_number": {
...
The `fields` object allows an anchor to describe fields that are passed into `POST /send`. It should include any KYC fields required for the sender or receiver, account information, and anything else required by the receiving anchor in order to complete a transaction.  Ideally the keys will be values from SEP-9 or one of the standard field names below to allow sending anchors to create a better UX.
...
Name | Type | Description
-----|-----|------
`require_receiver_info` | array | A list of SEP-9 values requested for the receiving client
