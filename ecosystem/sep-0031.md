...
* `sep12`: an object containing `sender` and `receiver` keys.
  * `sender`: an object containing a `types` key if KYC information is required for the sending client.
  ...
  * `receiver`: an object containing a `types` key if KYC information is required for the receiving client
