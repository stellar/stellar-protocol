...
2. If `pending_customer_info_update`, the SEP-9 KYC values collected were invalid and must be corrected by the Sending Client
   ...
   2. The Sending Anchor then reaches out the Sending Client again, collects valid values for the invalid fields, and makes a `PUT KYC_SERVER/customer` request to the Receiving Anchor
3. After providing the Receiving Anchor with updated values, the status should ultimately change to `completed`
