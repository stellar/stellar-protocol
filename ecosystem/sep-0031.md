...
```mermaid
stateDiagram-v2
    state "pending_sender" as ps
    state "pending_customer_info_update" as pciu
    state "pending_receiver" as pr
    state "pending_transaction_info_update" as ptiu
    state "pending_external" as pe
    state "pending_stellar" as pstr
    state "completed" as c
    state "error" as error
    

    [*] --> ps 
    ps --> pr : payment is received

    pr --> pciu: more customer info is required
    pciu --> pr: sep12/customer PUT
    
    pr --> pe: fund is being delivered as a off-chain/non-Stellar asset
    pe --> c: fund is delivered
    
    pr --> c: fund is delivered

    pr --> pstr: fund is being delivered as a Stellar asset
    pstr --> c: fund is delivered

    pr --> ptiu: more transaction info is required
    ptiu --> pr: sep31 /transactions PATCH
