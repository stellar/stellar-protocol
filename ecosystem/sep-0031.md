...
1. The Sending Anchor makes `GET /customer/transaction_id=<id>&type=<customer_type>` request to the Receiving Anchor for
   each of the Sending and Receiving Client. The response includes the customer `status` and the [SEP-9]
...
per-customer fields provided via [SEP-9] fields in [SEP-12 PUT /customer](sep-0012.md#customer-put)
requests.
...
an error response. This approach is deprecated in favor of using per-customer fields provided via [SEP-9]
