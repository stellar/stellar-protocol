...
`amount_in` | string | (optional) The amount of the Stellar asset received or to be received by the Receiving Anchor. Excludes any fees charged after Receiving Anchor receives the funds. </br></br> It should be equals to the `amount` value used in the [`POST /transactions`](#post-transactions) request. If a `quote_id` was provided in the [`POST /transactions`](#post-transactions) request, `quote.sell_amount` should also be equals to `amount_in`.
...
`amount_fee` | string | (optional) The amount of fee charged by the Receiving Anchor. If a `quote_id` was used, the SEP-31 `amount_fee` should be equals `quote.fee.total`.
`amount_fee_asset` | string | (optional) The asset in which fees are calculated in. Must be present if `quote_id` or `destination_asset` was included in the `POST /transactions` request. The value must be in [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format). If a `quote_id` was used, the SEP-31 `amount_fee_asset` should be equals `quote.fee.asset`.
...
```js
amount_fee_asset = sum(quote.fee.asset)
```
