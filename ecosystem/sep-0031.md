...
* `receiver_sep12_required`: a boolean value indicating whether or not the receiving anchor requires the recieving client to be registered via SEP-12. If `true`, the sending anchor should use the appropriate value from `sep12_types` in a [SEP-12 GET /customer?type=](https://github.com/stellar/stellar-protocol/blob/master/ecosystem/sep-0012.md#customer-get) request to determine which fields should be collected.
...
* `sep12_types`: (optional) an object containing the accepted values for the `type` parameter in [SEP-12](sep-0012.md) requests. Each key should map to an object value with a human-readable `description`. This field can be omitted from the response if no KYC is necessary for both the sender and receiver.
...
* `sender_sep12_type`: (**deprecated**, optional) the default value of the `type` parameter used in `SEP-12 GET /customer` requests for customers sending funds. This field can be omitted from the response if no KYC is necessary.
* `receiver_sep12_type`: (**deprecated**, optional) the default value of the `type` parameter used in `SEP-12 GET /customer` requests for customers receiving funds. This field can be omitted if no KYC is necessary.
