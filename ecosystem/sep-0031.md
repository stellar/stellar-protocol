...
1. If the off-chain payment cannot be received by the Receiving Client almost immediately, the transaction's status should be updated to `pending_external` until received.
...
* `v2.5.0`: Deprecate and make optional the `fields` object in the `GET /info response` ([#1387](https://github.com/stellar/stellar-protocol/pull/1387))
