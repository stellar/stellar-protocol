...
`amount_fee` | string | (optional) The amount of fee charged by the Receiving Anchor. If a `quote_id` was used and the quote provides `fee_details`, the SEP-31 `amount_fee` should be `sum(quote.fee_details.amount)`.
`amount_fee_asset` | string | (optional) The asset in which fees are calculated in. Must be present if `quote_id` or `destination_asset` was included in the `POST /transactions` request. The value must be in [SEP-38 Asset Identification Format](sep-0038.md#asset-identification-format). If a `quote_id` was used and the quote provides `fee_details`, the SEP-31 `amount_fee_asset` should be `any(quote.fee_details.asset)`, since SEP-38 does not allow more than one asset in the fee details.
...
* `v1.6.1`: App information about how a SEP-31 transaction will populate the `amount_fee` and `amount_fee_asset` fields when a `quote_id` is used. ([#1204](https://github.com/stellar/stellar-protocol/pull/1204))
