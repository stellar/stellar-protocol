...
`bldimg` is an open field that may reference any container image. This SEP places no restrictions on which images are permitted, but verifiers SHOULD carefully consider which images and image sources they accept, because the environment a contract build executes within is part of its supply chain. Verifiers are expected to maintain either an allowlist of images or an allowlist of image sources rather than accepting arbitrary references.
