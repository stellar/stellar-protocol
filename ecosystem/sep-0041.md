...
- or `map` containing:
  ...
  - `to_muxed_id: Option<u64 | String | Bytes<32>>` void is no muxed ID, or the
    u64 muxed ID, or for historical legacy reasons, Stellar assets also string
    and bytes.
...
- or `map` containing:
  - `amount: i128` the amount minted.
  - `to_muxed_id: Option<u64 | string | bytes>` void is no muxed ID, or the u64
    muxed ID, or for historical legacy reasons, Stellar assets also string and
    bytes.
