...
This SEP has purposely not added `mint()`, `init_asset()`, nor `clawback()` functions to give contracts more flexibility to implement and name these functions as they see fit. The contract is free to define how they want to `mint`/`clawback` a new custom token. The only requirement is for `mint` and `clawback` events to be emitted during minting and clawback actions.
