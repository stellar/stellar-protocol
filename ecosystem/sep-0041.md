...
- or `map` containing entries:
  - `amount: i128` the amount minted.
  - `to_muxed_id: Option<u64 | String | BytesN<32>>` void or absent if no muxed
    ID, or the u64 muxed ID, or for historical legacy reasons, the string or
    bytes memo for Stellar assets.
