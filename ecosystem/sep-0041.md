...
`clawback` events to be emitted during minting and clawback actions. While
implementations have this flexibility, the mint action must increase total
supply, and the clawback action must reduce total supply.
...
- `v0.4.1` - Clarify that clawback burns tokens reducing total supply and that no separate burn or transfer event is emitted alongside the clawback event.
