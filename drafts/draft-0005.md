...
A multisig account adds a data entry with the key `config.multisig.domain` and the value of a hostname used to resolve the `stellar.toml` file of the multisig service.

Example:
Key: `config.multisig.domain`
...
1. Look up the data entry with the key `config.multisig.domain` on the transaction source account. If the entry does not exist on the account, do not continue with this flow.
1. Resolve the `stellar.toml` associated with the account's `config.multisig.domain` entry and return the `MULTISIG_ENDPOINT` field.
