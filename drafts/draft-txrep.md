...
Txrep is a human-readable representation for Stellar transactions,
intended to be an "assembly language" of XDR.
...
Bug reports, test vectors, specifications of CAP semantics, and
Stellar documentation all need a way to talk concretely about the
contents of transactions.  Without a canonical human-readable format,
...
`[`, `]`, `.present?`, `.len`, or `_`) indicate the occurence of those
specific characters in the txrep source.  Lines then have the
following format:
