...
As in C, arrays are 0-based.  The _nested_pointer_field_ syntax is not
currently used because no fields contain pointers to pointers anywhere
in a `TransactionEnvelope`.  The syntax is simply reserved in the
unlikely event that transactions ever do contain such types, or the
more likely even that this specification is extended to cover
arbitrary XDR types beyond `TransactionEnvelope`.
...
Tailoring txrep specifically to XDR means it does not introduce a
