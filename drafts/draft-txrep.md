...
[XDR `TransactionEnvelope` data structure](https://github.com/stellar/stellar-core/blob/master/src/xdr/Stellar-transaction.x#L351).
The name is generated by joining XDR field names with a period (`.`),
similar to the syntax used for accessing nested C data structures.
However, every field name is capitalized, regardless of the
capitalization of the first character in the XDR file.  Arrays
elements (for both fixed- and variable-length arrays) are indexed
using square brackets.

Fields must precede any fields whose presence they affect.  In
particular, the line specifying an XDR union's discriminant value must
appear before the line describing the selected union field.
Similarly, as discussed below, the `.presence` and `.len`
pseudoselectors must precede any sibling selectors.
...
their state.  Lines contianing pseudoselectors must precede other
selectors of the same field.

Pointers use the `.present` pseudoselector with value `true` or
`false` to indicate that the field is present or NULL.  For example, a
transaction with timebounds might be specified like this (using
comments to annotate the times):
 ...
 * `string` values are represented as double-quoted interpreted string
   ...
   escapes (`"\xff"`), and the `"` and `\` characters can be escaped
   with another `\` (e.g., `"\\"`), and `\n` designates a newline.
...
strings, arrays, and variable-length `opaque`).
...
corresponding version byte:

| Key type | Version byte           |
| -------- | ---------------------- |
...
1. Compute a 16-bit CRC16 checksum of the version byte plus binary
   key (using polynomial x<sup>16</sup> + x<sup>12</sup> +
...
updates as the XDR files change.  Requiring union discriminants and
pseudoselectors to precede fields whose existence depends on them
makes it easy to parse txrep line-by-line and apply each value to a
`TransactionEnvelope` structure.

A small number of special cases (basically for keys and assets) make
the output easier for humans to process by providing compatibility
with other tools.

Field names were capitalized to avoid conflicting with
pseudoselectors, and because ecosystem software written in Go needs to
capitalize field names anyway.
