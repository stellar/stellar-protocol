...
Authors should draft proposals using [`slp-template.md`](slp-template.md). The Core team's
evaluation criteria — what each resource impacts, the ledger/transaction ratio, and how proposals
are benchmarked — are documented in the
[`SLP technical evaluation guide`](slp-technical-evaluation.md). This README focuses on process.
...
Note that while getting an SLP approved by the SLP committee increases the chance of it getting voted by the network, it does not guarantee its ratification when it comes to final vote performed by validators as that final vote depends on individual quorum configuration, and validator operator's opinion on the SLP at the time of the vote.
...
- **Impact Assessment** — To the extent possible, describe the expected impact of the change on the
  network and downstream systems (indexers, RPC providers, block explorers, etc.). Authors are not
  required to perform benchmarking; see the
  [technical evaluation guide](slp-technical-evaluation.md#impact-assessment) for the assessment
  the Core team will perform.
...
Once the proposal receives sufficient feedback from the community, the Core team will evaluate the
proposed numbers against the network's actual capabilities using the
[technical evaluation guide](slp-technical-evaluation.md). This includes:
...
### Awaiting Decision -> Accepted / Rejected
...
- You do **not** need to perform benchmarking yourself. The Core team will handle benchmarking and
  technical evaluation of proposed limit changes as part of the review process (see the
  [Core evaluation checklist](slp-technical-evaluation.md#core-evaluation-checklist)).
...
- If your proposal affects per-transaction limits, keep in mind that these increases are
  essentially irreversible (see
  [Reducing the limits](slp-technical-evaluation.md#reducing-the-limits)), so proposals should be
  well-justified.
